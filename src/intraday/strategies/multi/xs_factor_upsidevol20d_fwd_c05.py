"""xs_factor_upsidevol20d_fwd_c05 — auto-generated XS factor.

Signal: upside_vol_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_upside_vol_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorUpsidevol20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return ((sum(max(hist['close'][-i]/hist['close'][-i-1]-1.0, 0.0)**2 for i in range(1,21))/19)**0.5)
        except Exception:
            return None
