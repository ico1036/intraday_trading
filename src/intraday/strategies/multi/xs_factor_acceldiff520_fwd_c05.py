"""xs_factor_acceldiff520_fwd_c05 — auto-generated XS factor.

Signal: accel_diff_5_20  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_accel_diff_5_20_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAcceldiff520FwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return ((hist['close'][-1] - 2*hist['close'][-3] + hist['close'][-5]) / (hist['close'][-3] or 1e-9)) - ((hist['close'][-1] - 2*hist['close'][-11] + hist['close'][-21]) / (hist['close'][-11] or 1e-9))
        except Exception:
            return None
