"""xs_factor_atrproxy7d_rev_c05 — auto-generated XS factor.

Signal: atr_proxy_7d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_atr_proxy_7d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAtrproxy7dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 8:
            return None
        try:
            return sum(abs(hist['close'][-i] - hist['close'][-i-1]) for i in range(1,8))/7
        except Exception:
            return None
