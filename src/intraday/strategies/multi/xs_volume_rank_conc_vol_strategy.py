"""xs_vrev_conc_vol — concentration q + risk-parity weights (A+B combo).

Combines the two orthogonal enhancements:
    - Concentration: take only top/bottom ``concentration_pct`` by qv
    - Vol-target: per-coin weight ∝ 1/σ_coin within each leg

EDA on IS 2022-2024 (274-coin): sharpe 1.43 (= A alone), cum +77.3%
(= ~2x BASE). Mean roughly doubles (from concentration) AND variance
shrinks (from vol-target) — orthogonal effects.
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_vrev_conc_vol",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_vrev_variants.md"]


class XsVolumeRankConcVolStrategy:
    """Reverse-volume rank, concentration q + inverse-vol weighting."""
    ...
    def _rolling_vol(self, sym: str) -> float | None:
        ...
        if len(closes) < self.vol_lookback + 1: return None
        rets = []
    ...
    def _build_orders(self, state: MarketState) -> PortfolioOrder | None:
        # Eligible = has prev_qv + rolling vol.
        candidates = []
        vols = {}
        ...
        if len(candidates) < 4:
            return None
        candidates.sort(key=lambda t: t[1])  # ascending qv
        k = max(1, int(len(candidates) * self.concentration_pct))
        longs = [s for s, _ in candidates[:k]]    # low qv → long
        shorts = [s for s, _ in candidates[-k:]]  # high qv → short
