"""xs_factor_wq038tsopenxco_fwd_c10 — auto-generated XS factor.

Signal: wq_038_ts_open_x_co  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_038_ts_open_x_co_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq038tsopenxcoFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['open']) < 10:
            return None
        try:
            return -((sorted(hist['open'][-10:]).index(hist['open'][-1])+1)/10.0) * (hist['close'][-1]/(hist['open'][-1] or 1e-9))
        except Exception:
            return None
