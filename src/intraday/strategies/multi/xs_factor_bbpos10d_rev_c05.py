"""xs_factor_bbpos10d_rev_c05 — auto-generated XS factor.

Signal: bb_pos_10d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_bb_pos_10d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorBbpos10dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 10:
            return None
        try:
            return (hist['close'][-1] - sum(hist['close'][-10:])/10) / (2 * ((sum((c - sum(hist['close'][-10:])/10)**2 for c in hist['close'][-10:])/9)**0.5 or 1e-9))
        except Exception:
            return None
