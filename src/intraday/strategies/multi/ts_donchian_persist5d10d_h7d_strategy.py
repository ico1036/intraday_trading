"""is_085_ts_donchian_persist_5d10d_h7d — Donchian trend-filter with regime-persistent re-entry.

After a time-stop closes the position, if the slow regime is still in the same
direction, automatically re-enter on the next rebalance — no fresh fast
breakout required. Each hold_bars cycle becomes one trade.
"""
...
ALPHA_CELL = {
    ...
    "exit": "time_stop",
    "idea_family": "ts_donchian_persist_5d10d_h7d",
}
SOURCE_NOTES: list[str] = ["research/notes/ts_donchian_persist_5d10d_h7d.md"]


class TsDonchianPersist5d10dH7dStrategy:
    def __init__(self, symbols, fast_channel_bars=7200, slow_channel_bars=14400,
                 rebalance_bars=240, hold_bars=10080, max_weight=0.14, **_):
        ...
        if self._bar_count % self.rebalance_bars == 0:
            ...
            for s in self.symbols:
                ...
                if self._regime[s] != prev_reg:
                    self._has_traded[s] = False  # regime flip — require fresh trigger
                cs = self._side(state, s); reg = self._regime[s]
                # Initial entry: needs fast breakout AND regime agreement
                # Re-entry: regime active + no position + at least one prior trade
                if cs is None and reg > 0:
                    if (close >= fhi) or self._has_traded[s]:
                        cands.append((s, "LONG"))
                elif cs is None and reg < 0:
                    if (close <= flo) or self._has_traded[s]:
