"""xs_factor_wq054lcocratio_fwd_c10 — auto-generated XS factor.

Signal: wq_054_lc_oc_ratio  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_054_lc_oc_ratio_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq054lcocratioFwdC10(XsFactorBase):
    HISTORY_FIELDS = ('close', 'open', 'low', 'high')
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 1:
            return None
        try:
            return -((hist['low'][-1]-hist['close'][-1]) * (hist['open'][-1]**5)) / (((hist['low'][-1]-hist['high'][-1]) * (hist['close'][-1]**5)) or 1e-9)
        except Exception:
            return None
