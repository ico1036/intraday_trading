"""ORB-fade applied to a BTC+ETH pair only (pair universe variant).

Cell universe value differs from is_023 (basket_full → pair).
"""
...
class OrbFadePairStrategy:
    def __init__(
        ...
        leg_b: str = "ETHUSDT",
        or_minutes: int = 60,
        max_weight: float = 0.30,   # 2 active symbols, allow more concentration
        **_: Any,
    ):
        if not symbols:
