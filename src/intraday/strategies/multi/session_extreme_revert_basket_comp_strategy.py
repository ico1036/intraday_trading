"""Session extreme revert basket composite."""
...
class SessionExtremeRevertBasketCompStrategy:
    def __init__(
        self,
        symbols: list[str],
        warmup_minutes: int = 30,
        max_weight: float = 0.13,
        min_stretch_pct: float = 0.005,
        **_: Any,
    ):
        if not symbols:
        ...
        self.min_stretch_pct = float(min_stretch_pct)
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            stretch_up = (close - so) / so
            stretch_down = (so - close) / so
            # composite: position at extreme + magnitude
            if close >= sh - 1e-9 and stretch_up > self.min_stretch_pct:
                if cur != "SHORT":
                    orders[s] = Order(
                        side=Side.SELL, quantity=0.0,
                        weight=self.max_weight, order_type=OrderType.MARKET,
                    )
            elif close <= sl + 1e-9 and stretch_down > self.min_stretch_pct:
                if cur != "LONG":
                    orders[s] = Order(
