"""BB-fade session basket_full with composite transform."""
...
class BbFadeSessionBasketCompositeStrategy:
    def __init__(
        self,
        symbols: list[str],
        window: int = 1440,
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            band_dist = (close - mu) / sd
            mid_dist = (close - mu) / mu if mu > 0 else 0.0
            if band_dist > self.k:
                composite = band_dist + mid_dist * 100
