"""
ATR Volume Unit Risk Portfolio Strategy

Momentum strategy with volume-based entry confirmation
and ATR-based risk management across multiple crypto assets.

Core Signal - Direct Momentum with Volume Confirmation:
1. Price momentum identifies direction (trend following)
2. Volume filters false breakouts (liquidity confirmation)
3. ATR sizes risk appropriately (volatility adaptation)

Mathematical Foundation:
- Price Momentum = (Close_now - Close_lookback) / Close_lookback
- Volume Confirmation = Current Volume > Percentile(Volume_history, threshold)
- Volatility Regime = ATR_percentile < max_atr_percentile

Entry Signal = Momentum_direction AND Volume_confirmation AND Volatility_OK
...
Key Features:
- Simplified from VWAP divergence to direct price momentum
- momentum_threshold filter for entry quality
- Volatility regime filter (skip extreme ATR periods)
"""
...
class ATRVolumeUnitRiskMultiStrategy:
    """
    ATR + Volume-Confirmed Momentum Portfolio Strategy.

    Entry Conditions (Long):
    - Price momentum > momentum_threshold over lookback period
    - Current volume > min_volume_percentile of recent 20 bars
    ...
    def __init__(
        ...
        """
        Initialize the ATR Volume Unit Risk Portfolio Strategy.

        Args:
            symbols: List of trading symbols (min 2)
            lookback_minutes: Momentum calculation lookback period
            momentum_threshold: Minimum momentum for entry (default 0.2%)
    ...
    def _build_candidates(self, now: datetime) -> tuple[list[dict], list[dict]]:
        ...
        Filters by:
        - Price momentum calculation success
        ...
        for sym in self.symbols:
            ...
            momentum = self._price_momentum(sym, now)
            if momentum is None:
                skipped[sym] = "insufficient_momentum_data"
                continue

            # Check momentum threshold
            if abs(momentum) < self.momentum_threshold:
                skipped[sym] = f"momentum_below_threshold_{self.momentum_threshold}"
                continue
            ...
            rr = abs(momentum) / stop_pct if stop_pct > 0 else 0.0
            if rr < self.min_rr:
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"lookback={self.lookback_minutes}, momentum_thresh={self.momentum_threshold}, "
            f"top_n={self.top_n}, bottom_n={self.bottom_n}, atr_window={self.atr_window}, "
