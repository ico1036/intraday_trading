"""xs_factor_retkurt20d_fwd_c05 — auto-generated XS factor.

Signal: ret_kurt_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_ret_kurt_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRetkurt20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (lambda r=[hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,21)]: (sum((x - sum(r)/20)**4 for x in r)/20) / ((sum((x - sum(r)/20)**2 for x in r)/20)**2 or 1e-9))()
        except Exception:
            return None
