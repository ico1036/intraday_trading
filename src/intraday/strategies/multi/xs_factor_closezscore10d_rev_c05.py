"""xs_factor_closezscore10d_rev_c05 — auto-generated XS factor.

Signal: close_zscore_10d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_close_zscore_10d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorClosezscore10dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 10:
            return None
        try:
            return (hist['close'][-1] - sum(hist['close'][-10:])/10) / (((sum((c - sum(hist['close'][-10:])/10)**2 for c in hist['close'][-10:]))/9)**0.5 or 1e-9)
        except Exception:
            return None
