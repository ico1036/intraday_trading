"""ORB-fade with ewma_residual transform.

Trigger fades when close-vs-OR-boundary residual exceeds a fast EMA of past
residuals. Cell transform value differs (raw → ewma_residual).
"""
...
class OrbFadeEwmaResidualStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
        ...
        **_: Any,
        ...
        self._ema_mag: dict[str, float] = {s: 0.0 for s in self.symbols}
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            # Trigger only if instantaneous magnitude exceeds the EMA (residual > 0)
