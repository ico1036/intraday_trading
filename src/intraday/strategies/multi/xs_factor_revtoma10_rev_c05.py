"""xs_factor_revtoma10_rev_c05 — auto-generated XS factor.

Signal: rev_to_ma10  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_rev_to_ma10_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRevtoma10RevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 10:
            return None
        try:
            return hist['close'][-1]/(sum(hist['close'][-10:])/10) - 1.0
        except Exception:
            return None
