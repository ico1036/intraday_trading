"""xs_factor_mom121_rev_c05 — auto-generated XS factor.

Signal: mom_12_1  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_mom_12_1_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMom121RevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 85:
            return None
        try:
            return hist['close'][-8]/hist['close'][-85] - 1.0
        except Exception:
            return None
