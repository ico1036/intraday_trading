"""is_003 — orderflow reversion cross-section, sparse rebalance.

Hypothesis: extreme cross-sectional taker-buy aggression marks exhaustion;
the symbols with the most positive CVD-z mean-revert lower, those with the
most negative CVD-z mean-revert higher. Pair this with sparse rebalancing
to keep turnover low.

Differences vs is_002 (continuation):
    - Sign flipped: positive z is short, negative z is long.
    - rebalance_bars defaults to 60 (one decision per hour at TIME 60s bars).
    - entry_z defaults to 1.5 to act only on strong tails.
    - lookback_bars defaults to 120 (2h).
"""
...
class OrderflowReversionXsStrategy:
    def __init__(
        self,
        symbols: list[str],
