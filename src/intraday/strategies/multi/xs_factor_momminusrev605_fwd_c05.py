"""xs_factor_momminusrev605_fwd_c05 — auto-generated XS factor.

Signal: mom_minus_rev_60_5  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_mom_minus_rev_60_5_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMomminusrev605FwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 61:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-61] - 1.0) - (hist['close'][-1]/hist['close'][-6] - 1.0)
        except Exception:
            return None
