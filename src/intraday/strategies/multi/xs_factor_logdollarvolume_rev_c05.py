"""xs_factor_logdollarvolume_rev_c05 — auto-generated XS factor.

Signal: log_dollar_volume  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_log_dollar_volume_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorLogdollarvolumeRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['quote_volume']) < 1:
            return None
        try:
            return math.log(max(hist['quote_volume'][-1], 1e-12))
        except Exception:
            return None
