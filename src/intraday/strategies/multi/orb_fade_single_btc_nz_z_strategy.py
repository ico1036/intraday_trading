"""ORB-fade single BTC neutral_zone z_score (avoiding immediate close on entry)."""
...
class OrbFadeSingleBtcNzZStrategy:
    def __init__(
        self,
        symbols: list[str],
        target: str = "BTCUSDT",
        ...
        **_: Any,
        ...
        self.exit_inside_minutes = max(1, int(exit_inside_minutes))
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        # exit on close back inside OR for >= N minutes
        if cur is not None and self._or_low <= close <= self._or_high:
            self._inside_minutes += 1
            if self._inside_minutes >= self.exit_inside_minutes:
