"""xs_factor_volratio520_rev_c05 — auto-generated XS factor.

Signal: vol_ratio_5_20  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_ratio_5_20_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolratio520RevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (((sum((hist['close'][-i]/hist['close'][-i-1]-1.0)**2 for i in range(1,6))/4)**0.5)) / (((sum((hist['close'][-i]/hist['close'][-i-1]-1.0)**2 for i in range(1,21))/19)**0.5) or 1e-9)
        except Exception:
            return None
