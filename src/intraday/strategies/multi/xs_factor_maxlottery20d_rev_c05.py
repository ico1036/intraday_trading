"""xs_factor_maxlottery20d_rev_c05 — auto-generated XS factor.

Signal: max_lottery_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_max_lottery_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMaxlottery20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return max(hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,21))
        except Exception:
            return None
