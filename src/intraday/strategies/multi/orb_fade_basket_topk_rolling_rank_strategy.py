"""ORB-fade basket_topk with rolling_rank transform."""
...
class OrbFadeBasketTopkRollingRankStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            rank_val = sum(1 for x in hist if x <= mag) / max(1, len(hist))
            ranked.append((s, rank_val, side))
