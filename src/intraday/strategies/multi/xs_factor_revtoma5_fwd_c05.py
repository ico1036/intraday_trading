"""xs_factor_revtoma5_fwd_c05 — auto-generated XS factor.

Signal: rev_to_ma5  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_rev_to_ma5_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRevtoma5FwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 5:
            return None
        try:
            return hist['close'][-1]/(sum(hist['close'][-5:])/5) - 1.0
        except Exception:
            return None
