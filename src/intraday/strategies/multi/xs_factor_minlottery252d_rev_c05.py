"""xs_factor_minlottery252d_rev_c05 — auto-generated XS factor.

Signal: min_lottery_252d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_min_lottery_252d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMinlottery252dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 253:
            return None
        try:
            return min(hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,253))
        except Exception:
            return None
