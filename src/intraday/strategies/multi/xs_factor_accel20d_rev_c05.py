"""xs_factor_accel20d_rev_c05 — auto-generated XS factor.

Signal: accel_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_accel_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAccel20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (hist['close'][-1] - 2*hist['close'][-11] + hist['close'][-21]) / (hist['close'][-11] or 1e-9)
        except Exception:
            return None
