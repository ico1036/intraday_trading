"""BB-fade basket session NEUTRAL_ZONE exit z_score."""
...
class BbFadeBasketNzZscoreStrategy:
    def __init__(self, symbols, window=1440, k=2.0, neutral_z=0.5, rebalance_bars=60, max_weight=0.13, **_):
        self.symbols = [s.upper() for s in symbols]
        self.window = max(60, int(window)); self.k = float(k)
        self.neutral_z = float(neutral_z)
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            elif abs(z) < self.neutral_z and cur is not None:
