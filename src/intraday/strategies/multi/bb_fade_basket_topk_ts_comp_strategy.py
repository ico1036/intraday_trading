"""BB-fade basket_topk time_stop composite."""
...
class BbFadeBasketTopkTsCompStrategy:
    def __init__(self, symbols, window=1440, k=2.0, top_k=2, flat_at_minute=1410, rebalance_bars=60, max_weight=0.20, composite_threshold=0.10, **_):
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            if band_z > self.k:
                ...
                if composite > self.k + self.composite_threshold:
                    ranked.append((s, composite, "SHORT"))
            elif band_z < -self.k:
                composite = -band_z + (-mid_dist * 100)
                if composite > self.k + self.composite_threshold:
                    ranked.append((s, composite, "LONG"))
