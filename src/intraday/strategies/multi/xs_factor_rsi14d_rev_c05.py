"""xs_factor_rsi14d_rev_c05 — auto-generated XS factor.

Signal: rsi_14d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_rsi_14d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRsi14dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 15:
            return None
        try:
            return (lambda gains=[max(hist['close'][-i]-hist['close'][-i-1],0.0) for i in range(1,15)], losses=[max(hist['close'][-i-1]-hist['close'][-i],0.0) for i in range(1,15)]: (sum(gains)/14) / ((sum(gains)/14 + sum(losses)/14) or 1e-9))()
        except Exception:
            return None
