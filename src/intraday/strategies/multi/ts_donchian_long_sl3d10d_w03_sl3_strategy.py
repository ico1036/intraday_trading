"""is_150_ts_donchian_long_sl_3d10d_w03_sl3 — long-only persist Donchian with stop-loss."""
...
ALPHA_CELL = {
    ...
    "exit": "mixed",
    "idea_family": "ts_donchian_long_sl_3d10d_w03_sl3",
}
SOURCE_NOTES: list[str] = ["research/notes/ts_donchian_long_sl_3d10d_w03_sl3.md"]


class TsDonchianLongSl3d10dW03Sl3Strategy:
    def __init__(self, symbols, fast_channel_bars=4320, slow_channel_bars=14400,
                 rebalance_bars=240, hold_bars=20160, max_weight=0.03,
                 stop_loss_pct=0.03, **_):
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            opened_px = self._open_price.get(s)
            if cs == "LONG" and opened is not None:
                # Stop-loss check (every bar): drawdown from open price
                if opened_px is not None and self._closes[s]:
                    cur = self._closes[s][-1]
                    if (opened_px - cur) / opened_px >= self.stop_loss_pct:
                        orders[s] = Order(side=Side.SELL, quantity=0.0, order_type=OrderType.MARKET)
                        self._open_at.pop(s, None); self._open_price.pop(s, None); any_change = True
                        continue
                # Time-stop
        ...
        if self._bar_count % self.rebalance_bars == 0:
            ...
            for s in cands:
                ...
                if self._closes[s]:
                    self._open_price[s] = self._closes[s][-1]
