"""xs_factor_logretsum20d_rev_c05 — auto-generated XS factor.

Signal: log_ret_sum_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_log_ret_sum_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorLogretsum20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return sum(math.log(hist['close'][-i] / (hist['close'][-i-1] or 1e-9) or 1e-9) for i in range(1,21))
        except Exception:
            return None
