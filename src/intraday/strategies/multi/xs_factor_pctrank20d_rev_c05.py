"""xs_factor_pctrank20d_rev_c05 — auto-generated XS factor.

Signal: pct_rank_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_pct_rank_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorPctrank20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 20:
            return None
        try:
            return sum(1 for c in hist['close'][-20:] if c <= hist['close'][-1]) / 20.0
        except Exception:
            return None
