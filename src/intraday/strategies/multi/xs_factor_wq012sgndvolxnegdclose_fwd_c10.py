"""xs_factor_wq012sgndvolxnegdclose_fwd_c10 — auto-generated XS factor.

Signal: wq_012_sgn_dvol_x_neg_dclose  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_012_sgn_dvol_x_neg_dclose_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq012sgndvolxnegdcloseFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 2:
            return None
        try:
            return (1 if hist['volume'][-1]>hist['volume'][-2] else -1) * (-(hist['close'][-1]-hist['close'][-2]))
        except Exception:
            return None
