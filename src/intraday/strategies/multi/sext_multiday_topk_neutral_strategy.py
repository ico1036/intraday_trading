"""Session extreme revert multi_day topk neutral_zone."""
...
class SextMultidayTopkNeutralStrategy:
    def __init__(self, symbols, top_k=3, lookback_days=3, neutral_band=0.4, max_weight=0.25, **_):
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            snapshots[s] = (cl, mid, rw)
            mag = 0.0; tgt = None
            if cl >= roll_hi: mag = (cl - roll_hi) / rw + 0.001; tgt = "SHORT"
            elif cl <= roll_lo: mag = (roll_lo - cl) / rw + 0.001; tgt = "LONG"
        ...
        if state.positions:
            for s, info in state.positions.items():
                ...
                cl, mid, rw = snap
                if abs(cl - mid) <= self.neutral_band * rw:
