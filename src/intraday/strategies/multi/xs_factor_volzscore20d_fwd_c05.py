"""xs_factor_volzscore20d_fwd_c05 — auto-generated XS factor.

Signal: vol_zscore_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_zscore_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolzscore20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['quote_volume']) < 20:
            return None
        try:
            return ((hist['quote_volume'][-1] - (sum(hist['quote_volume'][-20:])/20)) / (((sum((v - sum(hist['quote_volume'][-20:])/20)**2 for v in hist['quote_volume'][-20:]))/19)**0.5 or 1e-9))
        except Exception:
            return None
