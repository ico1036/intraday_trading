"""xs_factor_retsignstreak5d_rev_c05 — auto-generated XS factor.

Signal: ret_sign_streak_5d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_ret_sign_streak_5d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRetsignstreak5dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 6:
            return None
        try:
            return sum((1.0 if hist['close'][-i] > hist['close'][-i-1] else -1.0) for i in range(1,6))
        except Exception:
            return None
