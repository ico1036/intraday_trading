"""ORB-fade multi_day basket trailing exit."""
...
class OrbMdBasketTrailingStrategy:
    def __init__(self, symbols, or_minutes=30, lookback_days=3, max_weight=0.13, trail_pct=0.015, **_):
    ...
    def generate_order(self, state):
        ...
        # Trailing exits processed every bar
        for s in self.symbols:
            d = state.panel.get(s)
            if not d: continue
            cl = d.get("close")
        ...
        for s in self.symbols:
            ...
            if self._side[s] is None:
                ...
                self._side[s] = tgt; self._best[s] = cl
        active = {s: o for s, o in orders.items() if o is not None}
        return PortfolioOrder(orders=orders) if active else None
