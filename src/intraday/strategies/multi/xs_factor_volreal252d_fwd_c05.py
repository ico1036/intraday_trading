"""xs_factor_volreal252d_fwd_c05 — auto-generated XS factor.

Signal: vol_real_252d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_real_252d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolreal252dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 253:
            return None
        try:
            return ((sum((hist['close'][-i]/hist['close'][-i-1]-1.0)**2 for i in range(1,253))/251)**0.5)
        except Exception:
            return None
