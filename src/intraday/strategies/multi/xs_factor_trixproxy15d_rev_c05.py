"""xs_factor_trixproxy15d_rev_c05 — auto-generated XS factor.

Signal: trix_proxy_15d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_trix_proxy_15d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorTrixproxy15dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 16:
            return None
        try:
            return (hist['close'][-1] - 3*hist['close'][-6] + 3*hist['close'][-11] - hist['close'][-16]) / (hist['close'][-1] or 1e-9)
        except Exception:
            return None
