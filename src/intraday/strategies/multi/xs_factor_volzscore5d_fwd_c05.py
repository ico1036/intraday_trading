"""xs_factor_volzscore5d_fwd_c05 — auto-generated XS factor.

Signal: vol_zscore_5d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_zscore_5d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolzscore5dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['quote_volume']) < 5:
            return None
        try:
            return ((hist['quote_volume'][-1] - (sum(hist['quote_volume'][-5:])/5)) / (((sum((v - sum(hist['quote_volume'][-5:])/5)**2 for v in hist['quote_volume'][-5:]))/4)**0.5 or 1e-9))
        except Exception:
            return None
