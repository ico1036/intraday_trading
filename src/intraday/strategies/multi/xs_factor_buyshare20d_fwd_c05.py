"""xs_factor_buyshare20d_fwd_c05 — auto-generated XS factor.

Signal: buy_share_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_buy_share_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorBuyshare20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['buy_volume']) < 20:
            return None
        try:
            return sum(hist['buy_volume'][-20:]) / (sum(hist['volume'][-20:]) or 1e-9)
        except Exception:
            return None
