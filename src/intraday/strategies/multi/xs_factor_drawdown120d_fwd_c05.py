"""xs_factor_drawdown120d_fwd_c05 — auto-generated XS factor.

Signal: drawdown_120d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_drawdown_120d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorDrawdown120dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 120:
            return None
        try:
            return hist['close'][-1] / max(hist['close'][-120:]) - 1.0
        except Exception:
            return None
