"""is_006_ts_weekly_reversal — per-symbol weekly time-series reversal.

Direction-opposite of ts_weekly_momentum (different idea_family).
"""
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "ts_weekly_reversal",
}
SOURCE_NOTES: list[str] = ["research/notes/ts_weekly_reversal.md"]


class TsWeeklyReversalStrategy:
    def __init__(
        self,
        symbols: list[str],
    ...
    def _z_for(self, symbol: str) -> float | None:
        ...
        if sigma <= 0 or not math.isfinite(sigma):
            return None
        # Reversal: invert the z-score so high momentum becomes a SHORT signal.
        return -(cur - mu) / sigma
