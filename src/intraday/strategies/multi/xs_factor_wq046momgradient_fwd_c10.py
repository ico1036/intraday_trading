"""xs_factor_wq046momgradient_fwd_c10 — auto-generated XS factor.

Signal: wq_046_mom_gradient  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_046_mom_gradient_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq046momgradientFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (lambda d1=hist['close'][-1]-hist['close'][-11], d2=hist['close'][-11]-hist['close'][-21]: (-1.0 if (d2-d1)/10 > 0.25 else (1.0 if (d2-d1)/10 < -0.25 else -(hist['close'][-1]-hist['close'][-2]))))()
        except Exception:
            return None
