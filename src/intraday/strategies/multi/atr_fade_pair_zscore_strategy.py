...
class AtrFadePairZscoreStrategy:
    def __init__(self, symbols, pair=("BTCUSDT", "ETHUSDT"), atr_window=60, k=1.0, history_size=30,
                 entry_z=0.5, max_weight=0.4, **_):
        ...
        self._tr_hist = {s: deque(maxlen=self.atr_window) for s in self.pair}
        self._closes = {s: deque(maxlen=2) for s in self.pair}
        self._mag_hist = {s: deque(maxlen=self.history_size) for s in self.pair}
        self._sma = {s: deque(maxlen=self.atr_window) for s in self.pair}
