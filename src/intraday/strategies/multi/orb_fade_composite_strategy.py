"""ORB-fade with composite transform (cell variant of is_023).

Combine OR break magnitude with OR-width-relative stretch to compose a
composite signal. Cell transform value differs (raw → composite).
"""
...
class OrbFadeCompositeStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
        ...
        **_: Any,
        ...
        self._or_open: dict[str, float | None] = {s: None for s in self.symbols}
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        if minute_of_day < self.or_minutes:
            for s in self.symbols:
                ...
                if self._or_open[s] is None and close is not None:
                    self._or_open[s] = float(close)
            # set midpoint at end of OR
            for s in self.symbols:
        ...
        for s in self.symbols:
            ...
            if close is None or hi is None or lo is None or mid is None:
                continue
            or_w = max(hi - lo, 1e-9)
            # composite: distance from OR boundary + distance from mid
            cur = self._current_side(state, s)
            if close > hi:
                composite = (close - hi) / or_w + (close - mid) / or_w
                ...
                composite = (lo - close) / or_w + (mid - close) / or_w
