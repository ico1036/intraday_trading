"""Per-symbol Donchian channel fade.

Compute rolling N-bar high and low. SHORT close > donchian-high; LONG close
< donchian-low; hold until OPPOSITE break (signal_flip).

Multi-day scale analogue of is_023 (orb_fade session/signal_flip).
"""
...
class DonchianFadeStrategy:
    def __init__(
        self,
        symbols: list[str],
        channel_bars: int = 1440,   # 24h window
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            if len(highs) < self.channel_bars + 1:
                continue
            # Donchian uses prior N bars excluding the current
            d_high = max(highs[-self.channel_bars - 1:-1])
            d_low = min(lows[-self.channel_bars - 1:-1])
            close = highs[-1]  # use latest high as recent price proxy; fall back to close in panel
