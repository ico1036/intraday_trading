"""xs_factor_force20d_fwd_c05 — auto-generated XS factor.

Signal: force_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_force_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorForce20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return sum((hist['close'][-i] - hist['close'][-i-1]) * hist['quote_volume'][-i] for i in range(1,21))
        except Exception:
            return None
