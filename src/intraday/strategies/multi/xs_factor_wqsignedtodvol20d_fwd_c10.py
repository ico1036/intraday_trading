"""xs_factor_wqsignedtodvol20d_fwd_c10 — auto-generated XS factor.

Signal: wq_signed_to_dvol_20d  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_signed_to_dvol_20d_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWqsignedtodvol20dFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-2]-1.0) / (((sum(min(hist['close'][-i]/hist['close'][-i-1]-1.0, 0.0)**2 for i in range(1,21))/19)**0.5) or 1e-9)
        except Exception:
            return None
