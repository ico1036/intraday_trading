"""ORB-fade applied to BTC only (single-asset universe variant).

Same family + exit as is_023 (orb_fade signal_flip session) but acts on a
single symbol only. Cell universe value differs (basket_full → single).
"""
...
class OrbFadeSingleBtcStrategy:
    def __init__(
        self,
        symbols: list[str],
        target: str = "BTCUSDT",
        or_minutes: int = 60,
        max_weight: float = 0.5,   # concentrate when single asset
        **_: Any,
    ):
        if not symbols:
