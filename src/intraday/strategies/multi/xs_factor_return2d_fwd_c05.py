"""xs_factor_return2d_fwd_c05 — auto-generated XS factor.

Signal: return_2d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_return_2d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorReturn2dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 3:
            return None
        try:
            return hist['close'][-1]/hist['close'][-3] - 1.0
        except Exception:
            return None
