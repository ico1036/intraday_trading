"""xs_factor_volxabsret5d_fwd_c05 — auto-generated XS factor.

Signal: vol_x_absret_5d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_x_absret_5d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolxabsret5dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 6:
            return None
        try:
            return sum(abs(hist['close'][-i]/hist['close'][-i-1]-1.0) * hist['quote_volume'][-i] for i in range(1,6))/5
        except Exception:
            return None
