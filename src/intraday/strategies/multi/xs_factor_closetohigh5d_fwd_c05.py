"""xs_factor_closetohigh5d_fwd_c05 — auto-generated XS factor.

Signal: close_to_high_5d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_close_to_high_5d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorClosetohigh5dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 5:
            return None
        try:
            return (hist['close'][-1] - min(hist['close'][-5:])) / (max(hist['close'][-5:]) - min(hist['close'][-5:]) or 1e-9)
        except Exception:
            return None
