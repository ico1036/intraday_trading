"""xs_factor_intradaygap1d_rev_c05 — auto-generated XS factor.

Signal: intraday_gap_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_intraday_gap_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorIntradaygap1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 1:
            return None
        try:
            return (hist['close'][-1] - hist['open'][-1]) / (hist['open'][-1] or 1e-9)
        except Exception:
            return None
