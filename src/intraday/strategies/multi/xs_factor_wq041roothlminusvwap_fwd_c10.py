"""xs_factor_wq041roothlminusvwap_fwd_c10 — auto-generated XS factor.

Signal: wq_041_root_hl_minus_vwap  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_041_root_hl_minus_vwap_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq041roothlminusvwapFwdC10(XsFactorBase):
    HISTORY_FIELDS = ('high', 'low', 'volume', 'quote_volume')
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['high']) < 1:
            return None
        try:
            return (hist['high'][-1]*hist['low'][-1])**0.5 - hist['quote_volume'][-1]/(hist['volume'][-1] or 1e-9)
        except Exception:
            return None
