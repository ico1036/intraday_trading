"""xs_factor_volgrowth5d_fwd_c05 — auto-generated XS factor.

Signal: vol_growth_5d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_growth_5d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolgrowth5dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['quote_volume']) < 6:
            return None
        try:
            return hist['quote_volume'][-1] / (sum(hist['quote_volume'][-6:-1])/5 or 1e-9) - 1.0
        except Exception:
            return None
