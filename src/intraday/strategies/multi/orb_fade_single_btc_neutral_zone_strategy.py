"""ORB-fade single BTC with neutral-zone exit (cell variant of is_040).

Position closes when close re-enters the OR range.
Cell exit value differs (signal_flip → neutral_zone).
"""
...
class OrbFadeSingleBtcNeutralZoneStrategy:
    def __init__(
        self,
        symbols: list[str],
        target: str = "BTCUSDT",
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        if close > self._or_high:
            ...
            # neutral zone — between OR-low and OR-high → close any open position
