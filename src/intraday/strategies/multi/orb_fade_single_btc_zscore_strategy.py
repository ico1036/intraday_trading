"""ORB-fade single BTC with z-score-magnitude trigger.

Cell transform value differs from is_040 (raw → z_score).
"""
...
class OrbFadeSingleBtcZscoreStrategy:
    def __init__(
        self,
        symbols: list[str],
        target: str = "BTCUSDT",
        or_minutes: int = 60,
        history_breaks: int = 30,
        entry_z: float = 0.5,
        ...
        **_: Any,
        ...
        self.history_breaks = max(5, int(history_breaks))
        ...
        self._break_mags: deque[float] = deque(maxlen=self.history_breaks)
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        or_width = max(self._or_high - self._or_low, 1e-9)

        if close > self._or_high:
            mag = (close - self._or_high) / or_width
            self._break_mags.append(mag)
            hist = list(self._break_mags)[:-1]
            if len(hist) >= 5:
            ...
            mag = (self._or_low - close) / or_width
            self._break_mags.append(mag)
            hist = list(self._break_mags)[:-1]
            if len(hist) >= 5:
                sd = pstdev(hist) or 1e-9
