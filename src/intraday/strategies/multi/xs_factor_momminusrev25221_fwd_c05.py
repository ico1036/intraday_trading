"""xs_factor_momminusrev25221_fwd_c05 — auto-generated XS factor.

Signal: mom_minus_rev_252_21  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_mom_minus_rev_252_21_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMomminusrev25221FwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 253:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-253] - 1.0) - (hist['close'][-1]/hist['close'][-22] - 1.0)
        except Exception:
            return None
