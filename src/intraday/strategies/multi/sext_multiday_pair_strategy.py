"""Session extreme revert multi_day pair."""
...
class SextMultidayPairStrategy:
    def __init__(self, symbols, pair=("BTCUSDT", "ETHUSDT"), lookback_days=3, max_weight=0.4, **_):
        ...
        self._highs = {s: deque(maxlen=self.window) for s in self.pair}
        self._lows = {s: deque(maxlen=self.window) for s in self.pair}
