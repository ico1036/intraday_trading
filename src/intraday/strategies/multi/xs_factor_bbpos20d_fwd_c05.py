"""xs_factor_bbpos20d_fwd_c05 — auto-generated XS factor.

Signal: bb_pos_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_bb_pos_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorBbpos20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 20:
            return None
        try:
            return (hist['close'][-1] - sum(hist['close'][-20:])/20) / (2 * ((sum((c - sum(hist['close'][-20:])/20)**2 for c in hist['close'][-20:])/19)**0.5 or 1e-9))
        except Exception:
            return None
