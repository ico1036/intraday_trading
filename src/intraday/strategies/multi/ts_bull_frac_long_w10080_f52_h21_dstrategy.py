"""is_720_ts_bull_frac_long_w10080_f52_h21d — bullish-bar fraction long."""
...
ALPHA_CELL = {
    ...
    "idea_family": "ts_bull_frac_long_w10080_f52_h21d",
}
SOURCE_NOTES: list[str] = ["research/notes/ts_bull_frac_long_w10080_f52_h21d.md"]


class TsBullFracLongW10080F52H21DStrategy:
    def __init__(self, symbols, window=10080, frac=0.52,
                 rebalance_bars=240, hold_bars=30240, max_weight=0.035, **_):
        if not symbols: raise ValueError("symbols")
        self.symbols = [s.upper() for s in symbols]
        self.window = max(60, int(window)); self.frac = float(frac)
        self.rebalance_bars = max(1, int(rebalance_bars))
        self.hold_bars = max(1, int(hold_bars))
        self.max_weight = max(0.0, min(1.0, float(max_weight)))
        self._bull = {s: deque(maxlen=self.window) for s in self.symbols}
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            self._bull[s].append(1 if float(c) > float(o) else 0)
        self._bar_count += 1
        ...
        if self._bar_count % self.rebalance_bars == 0:
            cands = []
            for s in self.symbols:
                if len(self._bull[s]) < self.window: continue
                frac = sum(self._bull[s]) / len(self._bull[s])
                cs = self._side(state, s)
                if cs is None and frac > self.frac:
                    cands.append(s)
