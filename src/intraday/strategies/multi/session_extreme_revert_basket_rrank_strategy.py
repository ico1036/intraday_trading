"""Session extreme revert basket rolling_rank."""
...
class SessionExtremeRevertBasketRrankStrategy:
    def __init__(
        self,
        symbols: list[str],
        ...
        **_: Any,
        ...
        self._stretch_hist: dict[str, deque[float]] = {
    ...
    def _reset(self) -> None:
        for s in self.symbols:
            # save previous day's stretch
            if self._sess_open[s] is not None and self._sess_high[s] is not None and self._sess_low[s] is not None:
                so = self._sess_open[s]
                sh = self._sess_high[s]
                sl = self._sess_low[s]
                stretch = max(abs(sh - so), abs(so - sl)) / max(so, 1e-9)
                self._stretch_hist[s].append(stretch)
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            if close is None or sh is None or sl is None or so is None or so <= 0:
                continue
            stretch = abs((close - so) / so)
            hist = list(self._stretch_hist[s])
            if len(hist) >= 5:
                rank = sum(1 for x in hist if x <= stretch) / len(hist)
