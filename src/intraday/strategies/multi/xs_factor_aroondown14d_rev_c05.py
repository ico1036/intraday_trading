"""xs_factor_aroondown14d_rev_c05 — auto-generated XS factor.

Signal: aroon_down_14d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_aroon_down_14d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAroondown14dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 14:
            return None
        try:
            return (14 - (hist['close'][-14:].index(min(hist['close'][-14:])))) / 14.0
        except Exception:
            return None
