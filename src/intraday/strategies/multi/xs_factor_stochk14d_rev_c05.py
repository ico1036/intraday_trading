"""xs_factor_stochk14d_rev_c05 — auto-generated XS factor.

Signal: stoch_k_14d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_stoch_k_14d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorStochk14dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 14:
            return None
        try:
            return (hist['close'][-1] - min(hist['close'][-14:])) / ((max(hist['close'][-14:]) - min(hist['close'][-14:])) or 1e-9)
        except Exception:
            return None
