"""xs_factor_intradaygap5d_rev_c05 — auto-generated XS factor.

Signal: intraday_gap_5d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_intraday_gap_5d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorIntradaygap5dRevC05(XsFactorBase):
    HISTORY_FIELDS = ('close', 'open')
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 5:
            return None
        try:
            return sum((hist['close'][-i] - hist['open'][-i]) / (hist['open'][-i] or 1e-9) for i in range(1,6))/5
        except Exception:
            return None
