"""xs_factor_momdiff1040_rev_c05 — auto-generated XS factor.

Signal: mom_diff_10_40  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_mom_diff_10_40_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorMomdiff1040RevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 41:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-11]-1.0) - (hist['close'][-1]/hist['close'][-41]-1.0)
        except Exception:
            return None
