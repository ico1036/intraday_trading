"""ORB-fade multi_day pair z_score (different cell from raw pair which had too few trades)."""
...
class OrbFadeMultidayPairZscoreStrategy:
    def __init__(self, symbols, pair=("BTCUSDT", "ETHUSDT"), or_minutes=30, lookback_days=3, history_size=30,
                 entry_z=0.0, max_weight=0.4, **_):
        ...
        self._or_high_today = {s: None for s in self.pair}
        self._or_low_today = {s: None for s in self.pair}
        self._past_or = {s: deque(maxlen=self.lookback_days) for s in self.pair}
