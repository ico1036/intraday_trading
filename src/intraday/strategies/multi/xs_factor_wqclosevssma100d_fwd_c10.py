"""xs_factor_wqclosevssma100d_fwd_c10 — auto-generated XS factor.

Signal: wq_close_vs_sma_100d  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_close_vs_sma_100d_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWqclosevssma100dFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 100:
            return None
        try:
            return hist['close'][-1] / (sum(hist['close'][-100:])/100 or 1e-9) - 1.0
        except Exception:
            return None
