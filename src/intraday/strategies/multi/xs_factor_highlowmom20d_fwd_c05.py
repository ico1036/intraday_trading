"""xs_factor_highlowmom20d_fwd_c05 — auto-generated XS factor.

Signal: high_low_mom_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_high_low_mom_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorHighlowmom20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['high']) < 20:
            return None
        try:
            return hist['high'][-1] / hist['low'][-20] - 1.0
        except Exception:
            return None
