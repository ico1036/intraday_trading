"""xs_factor_voladjret252d_rev_c05 — auto-generated XS factor.

Signal: vol_adj_ret_252d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_adj_ret_252d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVoladjret252dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 253:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-253]-1.0) / (((sum((hist['close'][-i]/hist['close'][-i-1]-1.0)**2 for i in range(1,253))/251)**0.5) or 1e-9)
        except Exception:
            return None
