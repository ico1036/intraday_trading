"""xs_factor_wq033openoverclose_fwd_c10 — auto-generated XS factor.

Signal: wq_033_open_over_close  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_033_open_over_close_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq033openovercloseFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['open']) < 1:
            return None
        try:
            return (hist['open'][-1]/(hist['close'][-1] or 1e-9)) - 1.0
        except Exception:
            return None
