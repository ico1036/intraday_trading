"""xs_factor_hightohigh60d_rev_c05 — auto-generated XS factor.

Signal: high_to_high_60d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_high_to_high_60d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorHightohigh60dRevC05(XsFactorBase):
    HISTORY_FIELDS = ('high',)
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['high']) < 60:
            return None
        try:
            return hist['high'][-1] / max(hist['high'][-60:]) - 1.0
        except Exception:
            return None
