"""xs_factor_adxproxy14d_rev_c05 — auto-generated XS factor.

Signal: adx_proxy_14d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_adx_proxy_14d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAdxproxy14dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 15:
            return None
        try:
            return (lambda up=sum(max(hist['close'][-i]-hist['close'][-i-1],0.0) for i in range(1,15)), dn=sum(max(hist['close'][-i-1]-hist['close'][-i],0.0) for i in range(1,15)): abs(up - dn) / ((up + dn) or 1e-9))()
        except Exception:
            return None
