"""xs_factor_atrproxy21d_fwd_c05 — auto-generated XS factor.

Signal: atr_proxy_21d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_atr_proxy_21d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorAtrproxy21dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 22:
            return None
        try:
            return sum(abs(hist['close'][-i] - hist['close'][-i-1]) for i in range(1,22))/21
        except Exception:
            return None
