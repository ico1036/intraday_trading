"""xs_factor_revstrength20d_rev_c05 — auto-generated XS factor.

Signal: rev_strength_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_rev_strength_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRevstrength20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 20:
            return None
        try:
            return (hist['close'][-1] - sum(hist['close'][-20:])/20) / (hist['close'][-1] or 1e-9)
        except Exception:
            return None
