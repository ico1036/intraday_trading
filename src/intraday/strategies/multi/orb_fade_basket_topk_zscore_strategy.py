"""ORB-fade basket_topk with z-score transform (variant of is_048).

Same universe/exit/horizon/family as is_048 but rank by z-score of break
magnitude vs symbol's own break-magnitude history. Cell transform differs
(raw → z_score).
"""
...
class OrbFadeBasketTopkZscoreStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        # compute signed z-score per symbol
        signed_z: list[tuple[str, float, str]] = []  # (sym, z, side)
        for s in self.symbols:
            d = state.panel.get(s)
            if not d:
                continue
            ...
            if len(hist) >= 5:
                ...
                z = 0.0
            signed_z.append((s, z, side))

        # Take top_k largest z magnitudes regardless of side
        signed_z.sort(key=lambda kv: kv[1], reverse=True)
        chosen = signed_z[: self.top_k]
        target: dict[str, str] = {sym: side for sym, _, side in chosen}
        ...
        for s in self.symbols:
            ...
            if tgt == "LONG":
                ...
                orders[s] = None  # signal_flip: hold
