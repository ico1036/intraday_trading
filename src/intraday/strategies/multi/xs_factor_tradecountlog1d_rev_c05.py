"""xs_factor_tradecountlog1d_rev_c05 — auto-generated XS factor.

Signal: trade_count_log_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_trade_count_log_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorTradecountlog1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['trade_count']) < 1:
            return None
        try:
            return math.log(max(hist['trade_count'][-1], 1))
        except Exception:
            return None
