"""is_516_xs_donchian_rank_c10d_k1_r7d — cross-section Donchian channel-position rank, long top-K / short bottom-K."""
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_donchian_rank_c10d_k1_r7d",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_donchian_rank.md"]


class XsDonchianRankC10dK1R7dStrategy:
    def __init__(self, symbols, channel_bars=14400, rebalance_bars=10080,
                 top_k=1, max_weight=0.2, **_):
        if not symbols: raise ValueError("symbols")
        self.symbols = [s.upper() for s in symbols]
        self.channel = max(2, int(channel_bars))
        self.rebalance_bars = max(1, int(rebalance_bars))
        self.top_k = max(1, int(top_k))
        self.max_weight = max(0.0, min(1.0, float(max_weight)))
        self._highs = {s: deque(maxlen=self.channel) for s in self.symbols}
        self._lows = {s: deque(maxlen=self.channel) for s in self.symbols}
        self._closes = {s: deque(maxlen=self.channel) for s in self.symbols}
        self._bar = 0
    ...
    def generate_order(self, state):
        ...
        for s in self.symbols:
            ...
            self._highs[s].append(float(h)); self._lows[s].append(float(l))
        ...
        scores = {}
        for s in self.symbols:
            if len(self._highs[s]) < self.channel: continue
