"""xs_factor_wqsignedpv5d_fwd_c10 — auto-generated XS factor.

Signal: wq_signed_pv_5d  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_signed_pv_5d_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWqsignedpv5dFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 6:
            return None
        try:
            return sum((1.0 if hist['close'][-i] > hist['close'][-i-1] else -1.0) * hist['volume'][-i] for i in range(1,6))/5
        except Exception:
            return None
