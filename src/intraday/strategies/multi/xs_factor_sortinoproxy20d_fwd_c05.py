"""xs_factor_sortinoproxy20d_fwd_c05 — auto-generated XS factor.

Signal: sortino_proxy_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_sortino_proxy_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorSortinoproxy20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (lambda r=[hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,21)]: (sum(r)/20) / ((sum(min(x,0.0)**2 for x in r)/19)**0.5 or 1e-9))()
        except Exception:
            return None
