"""xs_factor_rangeratio20d_fwd_c05 — auto-generated XS factor.

Signal: range_ratio_20d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_range_ratio_20d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRangeratio20dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 20:
            return None
        try:
            return sum((hist['high'][-i] - hist['low'][-i]) / (hist['close'][-i] or 1e-9) for i in range(1,21))/20
        except Exception:
            return None
