"""xs_factor_sharpeproxy20d_rev_c05 — auto-generated XS factor.

Signal: sharpe_proxy_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_sharpe_proxy_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorSharpeproxy20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (lambda r=[hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,21)]: (sum(r)/20) / ((sum((x - sum(r)/20)**2 for x in r)/19)**0.5 or 1e-9))()
        except Exception:
            return None
