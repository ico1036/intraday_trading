"""xs_factor_rangeratio1d_rev_c05 — auto-generated XS factor.

Signal: range_ratio_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_range_ratio_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRangeratio1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 1:
            return None
        try:
            return (hist['high'][-1] - hist['low'][-1]) / (hist['close'][-1] or 1e-9)
        except Exception:
            return None
