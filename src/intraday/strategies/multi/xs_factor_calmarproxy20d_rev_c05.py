"""xs_factor_calmarproxy20d_rev_c05 — auto-generated XS factor.

Signal: calmar_proxy_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_calmar_proxy_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorCalmarproxy20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-21]-1.0) / ((1 - min(hist['close'][-i]/max(hist['close'][-21:-i+1] or [1e-9]) for i in range(1,21))) or 1e-9)
        except Exception:
            return None
