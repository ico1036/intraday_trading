"""xs_factor_wq023highbreakrevert_fwd_c10 — auto-generated XS factor.

Signal: wq_023_high_break_revert  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_023_high_break_revert_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq023highbreakrevertFwdC10(XsFactorBase):
    HISTORY_FIELDS = ('high',)
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['high']) < 20:
            return None
        try:
            return (-(hist['high'][-1]-hist['high'][-3]) if sum(hist['high'][-20:])/20 < hist['high'][-1] else 0.0)
        except Exception:
            return None
