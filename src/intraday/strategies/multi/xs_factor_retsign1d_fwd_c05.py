"""xs_factor_retsign1d_fwd_c05 — auto-generated XS factor.

Signal: ret_sign_1d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_ret_sign_1d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRetsign1dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 2:
            return None
        try:
            return (1.0 if hist['close'][-1] > hist['close'][-2] else -1.0)
        except Exception:
            return None
