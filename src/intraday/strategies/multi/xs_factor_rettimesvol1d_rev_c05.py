"""xs_factor_rettimesvol1d_rev_c05 — auto-generated XS factor.

Signal: ret_times_vol_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_ret_times_vol_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRettimesvol1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 2:
            return None
        try:
            return (hist['close'][-1]/hist['close'][-2]-1.0) * hist['quote_volume'][-1]
        except Exception:
            return None
