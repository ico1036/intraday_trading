"""xs_factor_wq051graddelta_fwd_c10 — auto-generated XS factor.

Signal: wq_051_grad_delta  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_051_grad_delta_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWq051graddeltaFwdC10(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 21:
            return None
        try:
            return (lambda g=((hist['close'][-21]-hist['close'][-11])/10) - ((hist['close'][-11]-hist['close'][-1])/10): (1.0 if g < -0.05 else -(hist['close'][-1]-hist['close'][-2])))()
        except Exception:
            return None
