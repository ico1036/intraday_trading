"""xs_factor_overnightgap1d_rev_c05 — auto-generated XS factor.

Signal: overnight_gap_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_overnight_gap_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorOvernightgap1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 2:
            return None
        try:
            return hist['open'][-1] / (hist['close'][-2] or 1e-9) - 1.0
        except Exception:
            return None
