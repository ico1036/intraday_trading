"""xs_factor_volratio560_fwd_c05 — auto-generated XS factor.

Signal: vol_ratio_5_60  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vol_ratio_5_60_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVolratio560FwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['quote_volume']) < 60:
            return None
        try:
            return (sum(hist['quote_volume'][-5:])/5) / ((sum(hist['quote_volume'][-60:])/60) or 1e-9)
        except Exception:
            return None
