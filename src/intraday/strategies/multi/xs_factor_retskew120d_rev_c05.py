"""xs_factor_retskew120d_rev_c05 — auto-generated XS factor.

Signal: ret_skew_120d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_ret_skew_120d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorRetskew120dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 121:
            return None
        try:
            return (lambda r=[hist['close'][-i]/hist['close'][-i-1]-1.0 for i in range(1,121)]: (sum((x - sum(r)/120)**3 for x in r)/120) / ((sum((x - sum(r)/120)**2 for x in r)/120)**1.5 or 1e-9))()
        except Exception:
            return None
