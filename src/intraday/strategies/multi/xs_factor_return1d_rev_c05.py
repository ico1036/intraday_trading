"""xs_factor_return1d_rev_c05 — auto-generated XS factor.

Signal: return_1d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_return_1d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorReturn1dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 2:
            return None
        try:
            return hist['close'][-1]/hist['close'][-2] - 1.0
        except Exception:
            return None
