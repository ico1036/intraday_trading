"""xs_factor_distfromlow20d_rev_c05 — auto-generated XS factor.

Signal: dist_from_low_20d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_dist_from_low_20d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorDistfromlow20dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 20:
            return None
        try:
            return hist['close'][-1] / min(hist['close'][-20:]) - 1.0
        except Exception:
            return None
