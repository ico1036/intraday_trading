"""xs_factor_bouncefromlow60d_rev_c05 — auto-generated XS factor.

Signal: bounce_from_low_60d  direction=rev  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_bounce_from_low_60d_rev_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorBouncefromlow60dRevC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 60:
            return None
        try:
            return hist['close'][-1] / min(hist['close'][-60:]) - 1.0
        except Exception:
            return None
