"""xs_factor_vwapdist5d_fwd_c05 — auto-generated XS factor.

Signal: vwap_dist_5d  direction=fwd  concentration=0.05
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_vwap_dist_5d_fwd_c05",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorVwapdist5dFwdC05(XsFactorBase):
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['close']) < 6:
            return None
        try:
            return hist['close'][-1] / (sum(hist['close'][-i]*hist['quote_volume'][-i] for i in range(1,6)) / (sum(hist['quote_volume'][-i] for i in range(1,6)) or 1e-9)) - 1.0
        except Exception:
            return None
