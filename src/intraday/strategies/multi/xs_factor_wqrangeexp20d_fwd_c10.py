"""xs_factor_wqrangeexp20d_fwd_c10 — auto-generated XS factor.

Signal: wq_range_exp_20d  direction=fwd  concentration=0.1
...
ALPHA_CELL = {
    ...
    "exit": "signal_flip",
    "idea_family": "xs_factor_wq_range_exp_20d_fwd_c10",
}
SOURCE_NOTES: list[str] = ["research/notes/xs_factor_zoo.md"]


class XsFactorWqrangeexp20dFwdC10(XsFactorBase):
    HISTORY_FIELDS = ('high', 'low')
    HISTORY_LEN = 80
    ...
    def _compute_score(self, hist: dict[str, list[float]]) -> float | None:
        if len(hist['high']) < 20:
            return None
        try:
            return ((hist['high'][-1]-hist['low'][-1])) / ((sum(hist['high'][-i]-hist['low'][-i] for i in range(1,21))/20) or 1e-9)
        except Exception:
            return None
