...
class RegimeSplitAtrBreakoutStrategy:
    """User-requested strategy scaffold compatible with PortfolioBacktestRunner.

    Goal:
    터틀기반 전략을 만든다. 전체 트레이딩 자금의 1%를ATR 로 나 누것이 1 유닛이다. 동일 종목의 유닛 개수제한, 상관계수가 높은 종목의 유닛개수 제한을 건다. 청산조건도 노이즈보다 높은 수준의 ATR 조건을 건다. 진입조건도 다양하게 테스트 한다. 롱숏 가능하고 레버리지도 가능. 손익비 1.5이상 샤프 0.6이상 나올때까지 계속될때까지해
    """
