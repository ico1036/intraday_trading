"""ORB-fade with percentile transform (cell variant of is_023).

Trigger only when current break magnitude is in the top X percentile of
recent 20-day break history. Cell transform value differs (raw → percentile).
"""
...
class OrbFadePercentileStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
        history_size: int = 50,
        entry_pctile: float = 0.30,  # accept breaks above 30th percentile of history
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            if len(hist) >= 10:
                ...
                idx = int(self.entry_pctile * (len(hist_sorted) - 1))
