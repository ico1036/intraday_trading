"""ORB-fade with signal-flip exit (different cell from is_017).

Same trigger as is_017 (SHORT break above OR-high, LONG break below) but
hold position until the OPPOSITE break occurs (signal flip), instead of
flattening at session end. Across days, holds may persist multi-day.

Cell exit value differs from is_017 (time_stop → signal_flip).
"""
...
class OrbFadeSignalFlipStrategy:
    def __init__(
        self,
        symbols: list[str],
        or_minutes: int = 60,
    ...
    def generate_order(self, state: MarketState) -> PortfolioOrder | None:
        ...
        for s in self.symbols:
            ...
            # FADE: break above OR-high → SHORT (or flip to short)
            if close > hi:
                if cur != "SHORT":
                    orders[s] = Order(
