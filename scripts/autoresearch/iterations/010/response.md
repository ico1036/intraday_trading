Reasoning: iter 001 (raw Neumann tangency) ERRORED; iter 004 (BL with LW) ERRORED on `normalize_coefficients` being called with a non-dict. The user explicitly hinted at Neumann-series eigenvalue suppression on a "high-return / high-risk" composite, so I want to stay near that family but bring genuinely new structure that should generalize better OS than naive tangency:

1. **James-Stein mean shrinkage** (Stein 1956 / Efron-Morris 1973). Tangency is `Σ⁻¹μ`; OS Sharpe is dominated by mean-estimator noise, not Σ-estimator noise. Shrinking sample means toward the grand mean is the canonical Stein move and is *missing* from auto_001/002/004/008. This is what should rescue Neumann-tangency from in-sample over-fit.
2. **Neumann-series Σ⁻¹** with power-iteration-estimated λ_max and `α = 1 / (1.1·λ_max)` (so `‖I − αΣ‖_op < 1`, series convergent), K=6 truncation. This is the user's hint, but with the safety scaffolding iter 001 likely lacked.
3. **Ledoit-Wolf diagonal shrinkage** on Σ (via `shrink_cov`, shrinkage 0.20) BEFORE Neumann — this conditions Σ so K=6 already kills the noisy-eigenmode tail.
4. **Sharpe tilt** post-tangency (multiplicative 0.4 + 0.6·sharpe/max), per the user's "high-return focus" — concentrates capital on the strong members the tangency already chose rather than equalizing.
5. **Selection**: top-80 by IS Sharpe, correlation-dedup at ρ=0.80 (different from auto_001/003/006), then cap at 30 — fewer near-clones, more diversity than auto_009.
6. **Gross-exposure target 0.60** (well inside the 0.30–0.90 sweet spot the spec calls out).
7. **Helper-API hardening**: every `normalize_coefficients` call passes a `dict[str, float]` built explicitly from `cols`, signs applied via `apply_signs` on dicts — directly addresses both prior INVALID/ERRORED failure modes.

Citation: Ledoit-Wolf (2004) shrinkage + Stein (1956) mean shrinkage + Neumann series for matrix inverse (standard numerical-analysis identity, used in Markowitz under singular Σ — see Goto & Xu 2015).
