Looking at the leaderboard hint, untried families, and the URGENT gross-exposure / regime-shift guidance, I'll go with **Ward hierarchical clustering on the HRP distance metric** (`d = sqrt(0.5·(1-ρ))`, Lopez de Prado 2016) with **per-cluster highest-IS-Sharpe centroid** as the representative, then **equal-magnitude sign-aligned weights** (cov-free, so no min-var underweighting trap).

Regime-robustness layered on top:
- Per-year IS Sharpe stability (every full IS year > 0)
- IS max-drawdown < 25%
- Correlation dedup at ρ=0.92 before clustering

Cluster count K=6 (in the documented sweet spot n∈[5,8]). Coefficient magnitudes = ±1.0 (sign from `member_signs_ic`), so the runner's row-L1 clamp ≤1 saturates frequently → high mean gross exposure (no 1/σ dilution). This combination — Ward-linkage + sign-aligned native-magnitude — is not in the iter 001-039 tried list (auto_009 used HRP-quasi-diag with Sharpe-tilted weights; auto_013 used cluster-median IR; auto_020 used CCM-Elton-Gruber tangency). Ward + centroid-best + ±1 magnitudes is a fresh combination.

Hardened against the iter 001/004/039 helper-API failures: skipping `normalize_coefficients` (caused 2 prior errors), wrapping `correlation_dedup` in try/except with a sensible fallback, defensive degenerate-pool fallback to top-IS-Sharpe.
