I'll build iteration 79 around a genuinely fresh angle: **correlation-rank + Sharpe-rank greedy composite scoring** (option 3 from the cov-free menu, never attempted in tried list auto_001..022). This is cov-free — no matrix inversion, no 1/σ underweighting trap — and uses the two ingredients the leaderboard rewards: per-year IS-Sharpe stability and 20% drawdown discipline. Final coefficients are equal-weight (signed), then scaled 6× so the row-L1 cap in `_runner` actually binds and gross exposure saturates the budget instead of leaving 95% on the table.

**Mechanism / literature cited:**
- Composite rank scoring blends `rank_by_Sharpe` (exploit) and `rank_by_mean_|corr|_to_selected_set` (explore-orthogonal), greedy-additive — closely related to greedy maximum-relevance / minimum-redundancy (mRMR, Peng-Long-Ding 2005) adapted to portfolio member selection. Conceptually CSCV-informed (López de Prado & Lewis 2018) via the year-stability robustness gate, but with the cheap rank fusion as the working selection rule.
- Drawdown discipline + per-year stability filter follows the winning leaderboard pattern (n_members ∈ [5,8], DD < 20%, robust across regimes).
- Sign alignment via `member_signs_ic` ensures kept members contribute their deployable sign so the equal-weight aggregation adds constructively rather than cancels.
