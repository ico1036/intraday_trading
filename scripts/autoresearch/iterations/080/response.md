...
**Method:** HERC-inspired Ward cluster-centroid composite (Raffinot 2018, extending Lopez de Prado 2016 HRP). This is a **cov-free** composition method, which the brief flags as critical to escape the 1/σ underweighting trap that has limited prior tangency/min-var attempts to mean_row_l1 ≈ 0.05.

**Pipeline:**
1. Pull all submittable alpha IDs from `run_2026_05_c`.
2. Sign-align via `member_signs_ic` (deployable direction).
3. Filter on three regime-robustness criteria (the failure mode the brief identifies as dominant — IS→OS regime shift):
   - Positive IS Sharpe.
   - **Per-year stability:** Sharpe > 0 in *every* sub-year of IS (rules out alphas lucky in one regime).
   - **DD discipline:** max IS drawdown < 25% (rules out tail-luck winners).
4. `correlation_dedup` at ρ=0.85 (clones removed).
5. Ward hierarchical clustering on `d = sqrt(0.5·(1−ρ))` → cut at K=6 clusters.
6. **Cluster centroid** = highest-IS-Sharpe alpha in each cluster — gets diverse risk drivers without dilution.
7. Equal-weight (1/K), apply signs, L1-normalize, then **scale by 0.75** to target mean_row_l1 in [0.5, 0.8].

**Why this is fresh / different:** No prior iteration used Ward-distance HERC centroid selection combined with both year-stability AND DD-cap filters. Cov-free (no inversion → no underweighting). N=6 sits in the empirical sweet spot. Avoids the 9/9-failed Gram-Schmidt path.
