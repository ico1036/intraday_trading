Looking at this iteration, I need to avoid all 16 tried families and address the dominant failure (IS→OS regime shift). The user wants high-return/high-risk focus, but the feedback strongly suggests **concentrated top selection (k=4-12) with regime-conditional robustness filters** outperforms diversification.

**My angle (fresh territory):** *Regime-Conditional Drawdown-Disciplined Concentrated Tangency* — filter alphas to those with **positive Sharpe in EVERY IS sub-year** (2022/2023/2024) AND **max IS drawdown < 25%** (CDaR-like discipline), then sign-align via IC, correlation-dedup at 0.85, concentrate on **top-k=8** by IS Sharpe, and solve Ledoit-Wolf-shrunk tangency `w ∝ Σ⁻¹μ` with non-negativity clipping (since R is already in deployable space). Scale to **mean row L1 ≈ 0.70**.

This is distinct from prior attempts:
- Not stability-IR (auto_013) — uses per-year Sharpe sign-stability (regime conditioning)
- Not CSCV/PBO bootstrap (auto_011/015) — uses sub-year split (cheaper, regime-targeted)
- Not max-div / risk-parity / HRP / NCO — directly targets concentrated tangency
- Adds a **drawdown gate** which none of the prior 16 used as a hard filter

Citation: Ledoit & Wolf (2004) optimal shrinkage; CDaR/drawdown-disciplined selection (Chekhlov-Uryasev-Zabarankin 2005); regime-conditional Sharpe gating motivated by the documented 2022-2024 → 2024-2026 macro shift.
