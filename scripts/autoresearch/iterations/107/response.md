Looking at the feedback carefully:

- **Cov-FREE methods preferred** — every cov-inverse attempt got buried at gross ~0.05 (1/σ weighting trap).
- **Skip Gram-Schmidt** — failed 9/9 (residual-Sharpe ranking overfits IS).
- **Cluster centroid + equal-weight** is explicitly listed and hasn't been the named idea in recent rejections.
- **Year-stability filter + DD discipline** are listed as winning ingredients.
- **n_members ∈ [5, 8]** is the sweet spot.
- **normalize_coefficients** has caused crashes (iter 004) — skip it; use plain equal coefs of magnitude 1 so Σ|c|=K and let the runner's row-L1 clamp do its job.

Approach: Lopez de Prado (2016) HRP-style correlation distance `d_ij = √(0.5·(1−ρ))`, Ward linkage (Murtagh 1985), cut at K=6 clusters, pick the top-IS-Sharpe representative per cluster, equal-weight with a light IS-Sharpe tilt, IC-sign aligned. Filters: per-year IS Sharpe > 0 across all years present, max IS DD < 25%, raw IS Sharpe ≥ 0.5.
