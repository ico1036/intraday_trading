Picking a genuinely fresh angle: **greedy Gram-Schmidt residualization** — a cov-free, user-suggested method that bypasses the 1/σ-weighting trap responsible for the mean-row-L1≈0.05 ceiling. Order candidates by IS Sharpe, take the top one, then iteratively pick the next candidate whose OLS residual against the already-kept set has the highest Sharpe. This directly maximizes incremental orthogonal alpha rather than minimizing variance, so it does not suppress its own gross exposure. Combined with year-stability + DD discipline (per the leaderboard pattern) and post-normalization scale-up to target mean row L1 ≈ 0.6-0.7.
