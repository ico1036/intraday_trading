Looking at iteration 29, I need to pick a fresh idea family not yet tried (NCO, HRP, HERC, DRP, MaxDiv, Mean-CVaR, Fractional Kelly, James-Stein, CSCV, antibias, PC1-residualized, year-stable+Tangency, Mean-Semivariance/Sortino, CCM, STARR-CVaR, Calmar/Young, etc. are all consumed).

**Pick: Omega-ratio optimization (Keating & Shadwick 2002)** — a probability-weighted ratio of upside vs downside relative to a threshold. Two reasons it suits this run:

1. Omega is *threshold-aware* — unlike Sharpe/Sortino it captures the full upside/downside CDF, so it favors alphas whose IS distribution has a fat right tail with a controlled left tail. This is closer to "high-return / high-risk" as the user requested.
2. Combining Omega with a **per-year positive-Sharpe regime filter** + **drawdown discipline** addresses the dominant IS→OS regime-shift failure mode flagged in the brief.

Pipeline (concentrated, n≈6, mean-row-L1 ≈ 0.7 to hit the gross budget):

- Universe = `select_is_submittable("run_2026_05_c")` (fallback all)
- Sign-align via `member_signs_ic` before computing returns
- Drop alphas with any calendar year of negative Sharpe (regime-stability gate)
- Drop top-half by max IS drawdown (CDaR-inspired discipline)
- Rank surviving by Omega(τ=0), correlation-dedup at ρ=0.80
- Take top-6 by Omega → weight = (Ω−1)⁺ × 1/σ², normalize L1=1, scale to gross 0.70
- Reapply signs so the runner gets the correctly-signed RAW weights

No matrix inversion → avoids the helper-stack crashes seen in iter 1/4.
