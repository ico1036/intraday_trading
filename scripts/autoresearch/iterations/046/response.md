I'll design a fresh composite that combines constant-correlation Ledoit-Wolf shrinkage (a more robust shrinkage target than diagonal for highly correlated alpha returns) with a Neumann-series matrix inverse (avoids direct inversion of an ill-conditioned Σ), then take a long-only mean-variance tangency direction. The selection layer stacks the empirically-winning ingredients — drawdown discipline, year-stability across IS sub-regimes, IC-aware sign flip, ρ=0.85 dedup — concentrated to top-6 by Sortino. Coefficients are L1-normalized then scaled to mean gross ≈ 0.7 to stay in the sweet spot for return generation.

This pairs three peer-reviewed ingredients that haven't been combined in the tried list:
- Ledoit & Wolf 2004 constant-correlation target (better than diagonal target when alphas share a common-factor structure, which crypto alphas do)
- Neumann truncated-series inverse with α scaled by power-iteration λ_max (avoids singular Σ blow-ups that killed iter 001)
- Sortino (Sortino 1991) for selection ranking — penalizes downside, robust under heavy-tailed returns

Knobs aimed at the winning pattern: k=6, DD<20%, ρ=0.85, Neumann K=4, gross 0.7.
