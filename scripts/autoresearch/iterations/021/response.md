Looking at the rejection history and the user's hint, I'm picking a fresh angle that hasn't been tried yet:

**Idea — STARR (Reward-to-CVaR) selection + downside-volatility risk-parity on a tight n=7 regime-robust set.**

Why this is mathematically distinct from prior attempts:

1. **STARR ratio (Stoyanov & Rachev 2003)** — `E[r] / CVaR_α(r)` — penalizes *tail* losses rather than two-sided variance. This is different from auto_007 (mean-CVaR optimization), auto_018 (mean-semivariance tangency), and auto_011 (CSCV-bootstrap Sharpe). STARR is a *selection criterion*, not an optimization objective, so it composes cleanly with a simple weighting rule.

2. **Downside-volatility parity** for allocation: `c_i ∝ √STARR_i / σ_down,i`. The √STARR tilt is gentler than Sharpe-tilt — it caps the concentration on lucky IS tails (which is what bites in the IS→OS regime shift). σ_down (semideviation of negatives) is a stricter denominator than total σ; rewards alphas that lose small even when they fail.

3. **Regime-robustness gates retained** but tightened: per-year IS Sharpe > 0 across every year in IS and IS max-drawdown < 25%. Member count k=7 sits in the empirically winning sweet spot [4,12] (auto_002 n=6 / auto_003 n=10).

4. **Gross budget target ≈ 0.65** — explicit post-L1 scaling, the missing piece in early iterations whose mean row L1 collapsed under naive 1/N.

Avoiding past bugs:
- `normalize_coefficients` receives a **dict**, not ndarray (the auto_004 failure mode).
- `correlation_dedup` receives a DataFrame and a dict `keep_metric`.
- `apply_signs` receives two dicts; signed coefficients are produced *after* the magnitude computation on sign-flipped returns.
- Multi-tier fallback so `select_members` cannot return < 2 ids; `member_weights` never returns all-zero.
