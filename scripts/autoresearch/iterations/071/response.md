Looking at this task, I need to generate iteration 71 of the composite alpha researcher. Based on the strategy hints:

**Reasoning:**
- The leaderboard pattern favors n=5-8 concentrated, year-stable, DD-disciplined members
- The user has explicitly flagged that cov-based optimizers (tangency/MV) systematically under-scale gross exposure
- I'll use a **cluster-centroid (cov-free) approach** — Ward hierarchical clustering on correlation distance, pick the highest-IS-Sharpe centroid per cluster (HERC/Raffinot 2018 spirit), equal-weight, then vol-aware gross-exposure rescale to ~0.7
- Adding both per-year IS Sharpe stability (positive in every IS sub-year) AND max-DD < 20% filters — these were the two regime-robustness ingredients flagged as winners
- Sign-aligned via `member_signs_ic` so IC<0 alphas contribute their deployable sign

This is cov-free (avoids the 1/σ trap), regime-aware (year-stability + DD), concentrated (K=6 clusters), and includes the mandatory gross-exposure post-scale. Idea family: **HERC cluster-centroid w/ year-stability** — not in the tried list.
