I'm in plan mode, but this task is a pure text-generation request (the harness extracts my output, I don't write to disk or call tools). I'll respond with rationale + the single fenced `COMPOSITE_FILE` block as the spec requires.

---

### Rationale (iteration 11)

**Tried families so far** (auto_001–010): Neumann tangency, NCO+MP+detone, HERC, Black-Litterman, DRP detoned eigenbasis, Max-Diversification, Mean-CVaR, Fractional Kelly, HRP quasi-diag, James-Stein tangency. All of these lean on *covariance* (either inverse, eigen-decomposed, or clustered).

**Fresh angle for iter 11: selection-side robustness, not covariance gymnastics.**
After 10 attempts the covariance-optimization track has not produced a working composite (two of them errored on dict/array contract bugs). I take an orthogonal route: **Combinatorial Symmetric Cross-Validation–informed robust Sharpe** (Bailey, Borwein, López de Prado & Zhu, 2014, *The Probability of Backtest Overfitting*) applied as a member-selection rule, then a simple Sharpe-tilted concentration weighting — no matrix inversion, no eigen-clipping. The hypothesis is that the bottleneck in the previous attempts wasn't the weight scheme but selection-time overfitting to in-sample Sharpe noise: a clone that happens to win on the full IS window is rewarded the same as a genuinely robust performer. Bootstrapped lower-quantile Sharpe estimates the *downside-robust* edge each member would have under random training subsamples, which is a documented proxy for OS deflation.

Pipeline:
1. Pull all `select_is_submittable` ids for `run_2026_05_c`, IC-flip signs, load IS returns matrix `R`.
2. For each member, compute Sharpe on 32 random 50%-subsamples of the IS dates → take the **25th-percentile Sharpe** as the robustness score (CSCV-style — punishes members whose Sharpe collapses on half the data).
3. Rank by robust Sharpe → top 60 → `correlation_dedup` at 0.85 (keep_metric = robust Sharpe) → re-sort kept set → take top 18 (concentration, per the user's high-return / high-risk ask).
4. Weight ∝ (robust Sharpe)^1.5 (tilt toward the most-robust members without going single-name).
5. `normalize_coefficients` → ‖c‖₁ = 1 → scale by `TARGET_GROSS = 0.6` so mean row-L1 lands inside the documented healthy band [0.30, 0.90].
6. `apply_signs` at the end so the runner sees IC-aligned signed coefficients.

This deliberately avoids any matrix inversion (no pinv, no Neumann, no LW) — the iter-001/004 failures both came from helper-contract mistakes deep inside an optimization chain. Keep the surface small, make the selection rule the alpha.
