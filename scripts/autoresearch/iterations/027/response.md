...
Cited method: **Michaud (1998) Resampled Efficient Frontier** — bootstrap the IS returns matrix, solve a Ledoit-Wolf-shrunk tangency on each draw, average the weights. This is *averaging in weight space*, not parameter space, which is precisely the Michaud prescription for stabilizing mean-variance under noisy μ̂ and Σ̂.

Why this fits attempt 27:
- **Regime-shift defence (the dominant failure mode in iters 1–14):** I gate alphas on per-year IS Sharpe > 0 (must be positive in both 2022/2023 *and* 2024 sub-windows) and additive max-drawdown > −0.25. This rejects alphas that owe their IS Sharpe to a single regime.
- **Concentration over diversification (user-validated):** target N = 10 after IC-sign-alignment, |ρ|>0.80 dedup, and Sharpe-top truncation — the n=6–12 sweet spot from auto_002/003 winners.
- **Eigenvalue-divergence suppression (user's Neumann ask, done a different way):** LW shrinkage (α=0.20) toward the diagonal inside each bootstrap regularizes Σ⁻¹ without a fixed-truncation Neumann polynomial — equivalent in spirit (suppresses high-noise eigenmodes) but well-defined for any Σ. Falls back to `sla.pinvh` if Cholesky fails.
- **Gross-exposure budget:** `normalize_coefficients(·, "l1")` then × 0.70 to target mean row L1 ≈ 0.55–0.70 (avoid the < 0.20 anemia trap).
- **Helper-contract hygiene:** `normalize_coefficients` only ever receives a `dict[str, float]` (the cause of iter 004's crash); `apply_signs` is called once at the end to map from the sign-flipped basis back to the deployable basis.

Idea family is distinct from the 22 previously-tried autos (Michaud resampling has not appeared; HRP/HERC/NCO/DRP/max-div/BL/MV-CVaR/Kelly/James-Stein/PC1-residual/Calmar/Starr/CCM/anti-bias/CSCV all already attempted).
