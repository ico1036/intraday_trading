...
**Rationale.** The user explicitly asks for Neumann-series eigenvalue-divergence suppression on a *concentrated* high-return basket. Past attempts using Neumann inverse hit it with n=60 (auto_001, OS-eroded by dilution) or paired with James-Stein (auto_010). What hasn't been tried: Neumann-K3 on a *small* (k=6) basket pre-filtered for **regime stability** — the empirical hint that's worked best across iters 1–40. Auto_017 used Ledoit-Wolf shrinkage with that regime filter; I'll swap to truncated Neumann inverse (K=3 with α = 0.9/λ_max(Σ) via power iteration), keeping the year-stability + DD discipline + ρ=0.85 dedup framework.

**Mechanism.** Truncated Neumann series Σ⁻¹ ≈ α·Σₖ₌₀ᴷ (I − αΣ)ᵏ implicitly down-weights the highest-eigenvalue modes (which carry the most sample noise on small-T cov), giving a *biased but stable* tangency direction w = Σ⁻¹μ. With K=3 we keep only the leading three terms — strongly damps the spurious eigenmodes that drive in-sample overfit.

**Literature cites.** Neumann (1937) matrix inversion series; Lopez de Prado (2019, MLfAM) RMT denoising motivation for low-K truncation; Grinold-Kahn IC sign alignment.

**Regime filters** (kept from auto_017's winning pattern, tightened): per-year IS Sharpe > 0 in every sub-year (2022/2023/2024), max IS drawdown < 22% (tighter than 25%), correlation dedup at ρ=0.85, then top-6 by IS Sharpe (was top-8). Post-scale coefficients to mean row-L1 ≈ 0.7 to hit the gross-budget sweet spot.
