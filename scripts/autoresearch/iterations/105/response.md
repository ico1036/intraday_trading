**Rationale.** Iteration 105. The dominant failure mode flagged repeatedly: cov-based optimizers (tangency, min-var, NCO, HRP-inv-var, Neumann-tangency) push `w ∝ Σ⁻¹·μ` which is 1/σ-weighted, then `_runner.py`'s row-L1 clamp doesn't activate because `Σ_s|W| ≪ 1` already → mean row L1 collapses to ~0.05 and the composite leaves 95% of risk budget on the table. The user has explicitly listed cov-free composition methods as untried; auto_003 used HERC (risk-parity inside clusters); auto_013 used cluster-MEDIAN representative + eq-weight. Untouched: **cluster-TOP-Sharpe representative + unit native-leverage equal weighting** with regime-aware pre-filters.

Method: Raffinot-style hierarchical clustering on `dist = sqrt(0.5(1-ρ))` with Ward linkage (his 2017 *Hierarchical Clustering-Based Asset Allocation* paper introduces this exact distance), but at the cluster level take the **top-IS-Sharpe** alpha (not median, not inv-var blend), then equal-weight at `c_a = 1.0` with sign flipped by `member_signs_ic`. This deliberately skips covariance inversion, so the 1/σ collapse cannot occur. With `c_a = 1.0` per member and ~6 members, `Σ|c|=6`, so the runner's row-L1 clamp will engage and the panel will sit near `mean_row_l1 ≈ 0.6-1.0` — solidly inside the [0.3, 0.9] target band. Pre-filters before clustering: per-year Sharpe ≥ 0 in ≥2 IS sub-years (regime-stability), max IS drawdown ≤ 25% (tail discipline), correlation-dedup at ρ=0.85.

Method cited: **Hierarchical cluster-centroid allocation (Raffinot 2017)**, modified with top-Sharpe representative selection and equal native-leverage weighting to bypass the 1/σ collapse trap.
