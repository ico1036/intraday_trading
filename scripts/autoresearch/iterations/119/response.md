...
Prior cov-based optimizers in this run consistently dilute gross exposure (mean row L1 ≈ 0.05) and waste the risk budget; the cov-free hints in the spec point at *cluster-centroid + equal-weight* (Lopez de Prado MLAM 2019 §4, also Raffinot 2018 HERC family) as a genuinely different angle from Σ⁻¹·μ. The cov-free section's only failed sibling was greedy Gram-Schmidt; cluster centroid is explicitly endorsed and has not yet been run with the regime-robustness filters that lead the leaderboard.

Design choices distinct from `auto_013` ("cluster_median_repr_eqweigh"):

1. **Ward linkage on correlation distance** `d_ij = √(½(1−ρ_ij))` (Lopez de Prado HRP distance metric).
2. **TOP-IS-Sharpe centroid per cluster**, not median — median picks the average member; top picks the cluster's best representative, mirroring auto_002/003 "concentrate on best-of-best" lesson.
3. **Year-stability filter** — require positive Sharpe in every IS calendar year (regime-conditional robustness, the dominant failure mode per spec).
4. **Max-DD ≤ 25%** discipline.
5. **Correlation dedup ρ=0.85** before clustering to break clones.
6. **IC-sign alignment** via `member_signs_ic` so IC<0 members contribute their deployable direction.
7. **Aggressive gross-scaling**: rescale L1-normalized coefficients to hit estimated mean row L1 ≈ 0.70 — directly addresses the "URGENT: composite underperforms individual alphas" exposure-ceiling issue.
8. **K=6 clusters** — sits in the [5,8] leaderboard sweet spot.

Cov is used only as a similarity metric for Ward (no inversion), so there is no Σ⁻¹ to blow up; we don't need Neumann/MP/LW shrinkage. This is mathematically a cov-free combiner.
