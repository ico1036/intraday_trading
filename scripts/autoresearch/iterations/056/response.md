Looking at this iteration, I need to deploy a **cov-FREE** composition method (explicitly flagged as never-attempted), apply the **gross exposure rescaling** (the URGENT failure mode), use **n=6-8 concentrated members**, apply **per-year stability + DD discipline** (regime-robustness), and hit a genuinely new idea family.

**Chosen method**: Greedy Gram-Schmidt orthogonal residual-Sharpe composition. Each candidate's IS returns are regressed on the span of already-selected members; the residual Sharpe scores orthogonal new information (Lopez de Prado-style forward stepwise selection, no covariance inversion). Pre-filter with per-year stability + DD < 25% + correlation dedup at ρ=0.85. Then blend raw-IS Sharpe with residual Sharpe for coefficient magnitudes and **hard-scale by 10×** after L1 normalization to push mean row L1 into [0.5, 0.9].

This is mechanically distinct from any cov-based optimizer used so far — no Σ⁻¹, no eigenbasis projection, no shrinkage. The "1/σ-weighting trap" can't trigger because the optimizer never sees a covariance matrix.
