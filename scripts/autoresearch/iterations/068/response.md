Looking at this iteration 68, I need a fresh, cov-free approach. Prior cov-based optimizers leak gross exposure (mean row L1 ≈ 0.05). Gram-Schmidt failed 9/9. The user hints at **cluster-centroid equal-weight** — never attempted, naturally diversified without inverting Σ, and preserves native leverage.

**Method**: Ward hierarchical clustering on `(1-ρ)/2` correlation-distance (Lopez de Prado 2016 metric), one representative per cluster (highest IS Sharpe = centroid), equal-weight K=6, IC-sign aligned, then bootstrap-scale gross to ≈0.7.

**Regime-aware pre-filters** (per the leaderboard pattern):
- per-year IS Sharpe > 0 across 2022/2023/2024 (regime-stability)
- max IS drawdown < 25% (tail discipline)
- correlation dedup ρ > 0.85 (LdP near-clone removal)

This is genuinely novel territory in this run (no clustering+centroid attempts), avoids matrix inversion (no Neumann/LW/tangency 1/σ trap), and the K=6 concentration matches the leaderboard sweet spot (n∈[5,8]).
