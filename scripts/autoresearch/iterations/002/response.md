I'll build a Nested Clustered Optimization composite — fresh territory vs. the auto_001 Neumann attempt. NCO (Lopez de Prado 2019, *Machine Learning for Asset Managers*, Ch. 7) is the right answer when Σ is noisy and ill-conditioned, which is exactly what bit a single-shot tangency solve. The pipeline: (1) Marchenko-Pastur denoising clips bulk eigenvalues below `(1+√(N/T))²` to their mean (kills RMT noise modes); (2) detoning replaces the top eigenvalue with the bulk mean (neutralizes the dominant common factor that would otherwise consume all the budget); (3) hierarchical clustering on `√(0.5(1-ρ))` partitions members into nearly-independent risk groups; (4) intra-cluster tangency `Σ⁻¹μ` (with trace-scaled Tikhonov + `pinvh` fallback) finds local max-Sharpe; (5) inter-cluster tangency on cluster portfolios composes them; (6) final coefficients are L1-normalized then multiplied by a 1.6 gross-budget factor to keep mean row L1 in the 0.5-0.7 sweet spot the spec calls for.

Sign-alignment via `member_signs_ic` happens *before* the returns matrix so denoising and Sharpe ranking see deployable polarities. Top-80-by-IS-Sharpe pre-filter + |ρ|>0.80 dedup keeps the problem well-posed for MP (q = N/T small) while still letting cluster diversity emerge. Defensive checks throughout to avoid the silent failure that errored auto_001.
