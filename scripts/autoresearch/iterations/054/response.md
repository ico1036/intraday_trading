Looking at this iteration, I need a genuinely new angle. Surveying tried families (auto_001-022): Neumann, NCO, HERC, BL, DRP, MaxDivR, Mean-CVaR, Fractional Kelly, HRP, James-Stein, CSCV, Antibias, Stability-IR cluster, Bootstrap ERC, PBO+Spinu, PC1 residual, regime year-stable+tangency, mean-semivariance, hedge+macro residual, CCM Elton-Gruber, STARR-CVaR, Calmar-Young91 — all of these except antibias are **cov-based**. The user's URGENT note flags cov-inversion as the gross-exposure trap.

**Method: Greedy Gram-Schmidt orthogonal addition by residual Sharpe** (Feng-Giglio-Xiu 2020, *"Taming the Factor Zoo"*, JF). This is explicitly listed as untried and cov-free. Mechanism:

1. Pre-filter alpha pool by IC-sign-aligned per-year stability (positive Sharpe in 2022, 2023, 2024 IS sub-windows) and IS max-DD ≤ 20% (regime robustness, per the empirical leaderboard pattern).
2. Correlation-dedup at ρ=0.90, keeping highest IS Sharpe representative.
3. Seed with highest IS-Sharpe member. Iteratively project each candidate's IS return series onto the linear span of already-selected members; rank candidates by the **residual** annualized Sharpe (information genuinely orthogonal to existing portfolio). Add the top one; stop at n=8 or when residual Sharpe drops below 0.30.
4. Weight by residual Sharpe (concentrates exposure on members that add new alpha, not dilute), apply IC sign alignment, L1-normalize, and scale up by 12× so the composite mean row L1 sits in [0.5, 0.9] (escaping the 1/σ underweighting trap diagnosed in the prompt — clamping handles any overshoot).

This is genuinely novel for this run: it never inverts Σ, so it doesn't downweight by `1/σ`; it builds the portfolio along residual-information axes (Gram-Schmidt), which is the cov-free analog of NCO without paying the cov-inversion penalty.
