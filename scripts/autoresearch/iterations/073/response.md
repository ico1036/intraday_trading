I'll cite **Ward hierarchical clustering on correlation distance** (Murtagh & Legendre 2014; Raffinot 2018 cluster-representative variant of HRP) with explicit regime-conditional filtering. This is **cov-FREE** (the prompt's flagged blocker) — no matrix inversion at all. The selection logic combines three regime-aware filters from the prompt's empirical winners:

1. **Per-year IS Sharpe positivity** (regime stability across 2022/2023/2024)
2. **Max IS drawdown < 20%** (drawdown discipline)
3. **Correlation dedup at ρ=0.85** (remove near-clones)

Then Ward clustering with `d = sqrt(0.5 * (1 - ρ))` on what remains, cut into **K=6** clusters, pick the highest-IS-Sharpe representative per cluster. Equal-weight after `member_signs_ic` flips, then `normalize_coefficients(c, "l1")` followed by a **fixed 3× scale** to push sum|c|≈3 so the combined panel exercises the gross-exposure budget rather than starving at row-L1 ≈ 0.05 (the dominant failure mode in iters 1-72).

This is a fresh idea family — auto_013 used "cluster_median_repr" but with different distance, no Ward, no year-stability+DD combo, no explicit gross-scaling. The user explicitly tagged "Cluster centroid then equal-weight" as never-attempted in the prompt menu.
