I'll generate attempt #118 using a cov-FREE method that hasn't been tried: **correlation-rank + IS-Sharpe-rank greedy selection** with year-stability and drawdown discipline pre-filters, then equal-weight rescaled to target mean gross ≈ 0.6.

Rationale:
- Cov-based methods (tangency / min-var / NCO / HRP / DRP) have all hit the 1/σ-weighting trap → mean row L1 ≈ 0.05.
- Gram-Schmidt failed 9/9 — explicitly avoided.
- Rank-based greedy selection (inspired by Choueifaty's diversification idea, but in rank space, no cov inversion) preserves native leverage. At each step I pick the candidate that maximizes `0.5·rank_sharpe_desc + 0.5·rank_meanabscorr_asc` — i.e., high IS Sharpe AND orthogonal to already-picked set.
- Pre-filters target regime robustness: positive Sharpe in EVERY IS calendar year, IS max drawdown < 20%.
- n=6 concentrated (leaderboard pattern shows n∈[5,8] wins).
- IC-aligned signs.
- After equal-weight + L1 norm (`Σ|c|=1`), apply explicit σ-weighted gross rescale to land mean row L1 ≈ 0.6 (the empirically demonstrated leverage gap).
