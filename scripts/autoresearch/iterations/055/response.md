Rationale: covariance-based optimizers have repeatedly produced under-leveraged composites (mean row-L1 ≈ 0.05 → OS ~0.4). I'm switching to a covariance-free **Greedy Gram-Schmidt orthogonal residual selection** (Pati–Rezaiifar–Krishnaprasad 1993 / Mallat–Zhang 1993 orthogonal matching pursuit), gated by per-year IS Sharpe stability and a 20% drawdown ceiling to defend against the IS→OS regime shift. Members are weighted by IS Sharpe (not 1/σ), then explicitly scaled up to occupy the gross-exposure budget. No Σ⁻¹ anywhere — sidesteps the eigenvalue-divergence trap entirely.
