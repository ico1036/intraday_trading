**Rationale.** This iteration combines three ingredients that are *novel together* in the tried-list: (1) the **Gerber statistic** (Gerber, Markowitz, Pujara 2022, *J. Portfolio Management*) — a count-based robust co-movement measure that down-weights the small joint movements that drive noise in Pearson correlation on heavy-tailed crypto returns, particularly important when IS spans LUNA/FTX tail events; (2) a **Neumann-series inverse** (truncated at K=5 with α=0.9/λ_max via power iteration) which honors the user's explicit MinVol-suppression ask while sidestepping the auto_001 failure mode (Neumann was paired with sloppy dict-conversion there); and (3) a **regime-aware prefilter** that keeps only year-stable (positive Sharpe in every IS sub-year), drawdown-disciplined (max IS DD < 20%) alphas, then concentrates on K=6 sharpe-tilted picks after ρ=0.82 dedup. The MinVol step uses tangency `w ∝ Σ⁻¹ μ` with μ = clipped IS-Sharpe and a coefficient post-scale targeting mean row-L1 ≈ 0.7 — matching the leaderboard sweet-spot pattern (n_members 5-8, year-stable, DD<25%, Neumann/MP cov, dedup 0.80-0.90). The Gerber comatrix is projected to nearest PSD via eigenvalue clipping before scaling by sample stds, ensuring `S` is well-defined for Neumann iteration.
