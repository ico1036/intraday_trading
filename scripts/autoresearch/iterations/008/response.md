Reasoning before code block:

**Iteration 8 — fresh family selection.** Tried families to avoid: Neumann, NCO+MP denoise+detone, HERC-Raffinot, Black-Litterman+LW, DRP eigenbasis, Max-Diversification, Mean-CVaR. Fresh territory: **Fractional Kelly (Kelly 1956; Thorp 1966; MacLean-Thorp-Ziemba 2010; Lopez de Prado 2018, AFML ch. 16)** with Ledoit-Wolf shrinkage on the inverse-covariance step. This is genuinely new in the run and mathematically rigorous.

**Why Kelly here:** the user's explicit ask is "high-return / high-risk alpha-focused combination" — Kelly maximizes expected log-wealth and naturally concentrates weight on members with high `μ/σ` and low cross-correlation, *without* artificially equal-weighting many near-orthogonal alphas (which past attempts showed dilutes mean row-L1 to <0.20 and produces anemic returns). The full-Kelly vector `w* = Σ⁻¹μ` is the same direction as the tangency portfolio but Kelly framing makes the "aggressive growth" interpretation explicit, and fractional Kelly (α=0.30) is the standard practitioner safeguard, although here it's absorbed into the L1-renormalization (kept as the conceptual label).

**Stability:** LW-style shrinkage via `shrink_cov(R, 0.20)` regularizes Σ; symmetric pseudo-inverse via `sla.pinvh` instead of explicit inversion. Selection: top-by-IS-Sharpe → correlation-dedup at |ρ|>0.85 (keeping by IS Sharpe) → cap at 18 (in the spec's recommended 10-30 band). Sign-align via IC before R-matrix construction; apply signs again after Kelly to recover deployable coefficients on the original W_a streams. L1-renormalize to target mean row-L1 ≈ 0.65 (mid of recommended 0.30–0.90 band).

**Bug-avoidance vs. iter 001 / 004 errors:** both errored inside `_optim_helpers.py`, with iter 004's tail showing `normalize_coefficients(s, "l1")` — almost certainly an ndarray vs dict mismatch. Every `normalize_coefficients` call here passes a `dict[str, float]`. Every `apply_signs` call also passes dicts. Defensive guards: degenerate-inverse fallback to Sharpe-proportional weights, total-mass guard before L1 normalization, and `R.shape[1] < 2` short-circuits.
