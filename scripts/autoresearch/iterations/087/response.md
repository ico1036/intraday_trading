Rationale: Pure cov-based optimizers in prior iterations created the gross-exposure ceiling (mean row L1 ≈ 0.05). Per the URGENT note, I'm going **cov-free** this iteration. I pick the **"Correlation-rank + Sharpe-rank greedy composite score"** path (not the failed Gram-Schmidt, not centroid-equal-weight) combined with the regime-aware filters that the leaderboard pattern confirms work: **per-year IS Sharpe stability**, **max IS drawdown ≤ 25%**, **N=6 concentrated**, **ρ=0.85 dedup**, **sign-aligned via IC**.

Mechanism: starting from the highest-IS-Sharpe survivor, greedy-add members minimizing `0.5·rank_sharpe + 0.5·rank_mean_abs_corr_to_selected`. This is the Asness-Frazzini-Pedersen rank-composite style adapted to portfolio construction (cf. also Bayer & Stahl 2019, correlation-distance portfolios). Avoids `Σ⁻¹·μ` entirely, so no 1/σ underweighting. Equal sign-embedded magnitude (|c|=1.0) per member ⇒ Σ|c|=6 ⇒ runner's row-L1 clamp engages with enough headroom to produce mean row L1 in the productive 0.4–0.8 band rather than the anemic <0.1 trap.

Regime defense: year-stability filter (positive Sharpe in EVERY IS sub-year) drops alphas that depended on a single regime; DD≤25% drops tail-luck alphas; greedy orthogonality prevents concentration in one strategy family.
