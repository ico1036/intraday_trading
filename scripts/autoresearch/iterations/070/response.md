Looking at this iteration 70, the key signals from the spec are:

1. **Cov-based optimizers consistently under-weight by 10×** → mean row L1 ≈ 0.05-0.25, leaving 95% of risk budget unused. The spec urgently flags cov-FREE methods.
2. **Gram-Schmidt failed 9/9** — explicitly told NOT to pick that of the three cov-free methods.
3. **Regime shift IS→OS dominates failures** — per-year IS Sharpe stability + DD discipline are repeated winners.
4. **n_members ∈ [5, 8]** is the sweet spot.

My pick: **Ward hierarchical clustering on correlation distance → 1-per-cluster top-IS centroid → cov-FREE equal weight (1/K) → L1 scale to ~1.2 for healthy gross**. This is the "Cluster centroid then equal-weight" method from the menu (which is in the cov-free list but NOT Gram-Schmidt). Pre-filter with per-year IS Sharpe positivity (2022/2023/2024) and IS max-DD < 22% to suppress regime-fragile picks (yen-carry / FTX / LUNA-era tails).

Why I expect this to beat prior tries: previous Ward-style ideas (HRP iter 009, HERC iter 003) were tied to recursive *risk-budgeting* inside clusters — those still apply 1/σ weighting and so still underweight gross. Pure 1/K on K=6 cluster centroids skips inv-cov entirely, preserving each member's native deployable scale. Combined with the stability filter, it targets the "concentrated robust" regime the leaderboard rewards.

Citation: Lopez de Prado (2016, *Building Diversified Portfolios that Outperform Out of Sample*) — correlation-distance metric d_ij = √(½(1-ρ_ij)) with hierarchical clustering; we keep the clustering step but replace recursive bisection with simple 1-per-cluster centroid + equal weight (Raffinot 2018-style at K=fixed).
