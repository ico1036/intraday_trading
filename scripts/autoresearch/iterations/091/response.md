Looking at this iteration 91, I need to break from cov-based optimization that's been failing repeatedly (1/σ-weighting trap producing 5% gross exposure). The prompt explicitly suggests cov-free methods.

**Strategy:** Correlation-rank + Sharpe-rank greedy composite — a cov-free, rank-aggregation approach. This bypasses Σ⁻¹·μ entirely, preserving native member leverage. Combined with regime-aware gating (year-stability across 2022/2023/2024 + DD<25%) per leaderboard pattern (n=5-8, year-stable, DD-discipline lands top-5).

**Mechanism:**
1. SUBMITTABLE pool → filter alphas with positive Sharpe in EVERY IS sub-year (regime robustness)
2. Drop alphas with max IS DD > 25% (lucky-tail screen)
3. Correlation dedup at ρ=0.85
4. Greedy pick n=6: combined rank score = 0.5·rank(IS-Sharpe desc) + 0.5·rank(mean|corr| with picks, asc)
5. Sign-align via IC (`member_signs_ic`)
6. Equal-weight, then scale by 4× to escape the gross-exposure trap (1/6 ≈ 0.167 → 0.67 per member → composite mean row L1 in [0.5, 0.8])

**Citation:** Greedy rank-aggregation diversification — Choueifaty-Coignard *Maximum Diversification Ratio* (2008) intent (orthogonal/diverse exposure), but rank-formulated and cov-free to avoid the variance-weighting that diluted prior attempts. Year-stability + DD gates per López de Prado backtest robustness heuristics.

This is genuinely fresh: not in the tried-list (no rank-aggregation), not Gram-Schmidt (failed 9/9), not pure 1/N (too generic), and addresses the gross-exposure ceiling directly.
