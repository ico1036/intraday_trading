I'll pick **HRP (Lopez de Prado 2016)** — correlation-distance single-linkage clustering, quasi-diagonalization, recursive bisection — none of the prior 8 attempts used canonical HRP (auto_003 was HERC, an HRP *extension* with different intra-cluster mechanics). Two strong reasons it fits this mandate:

1. **No matrix inversion** — auto_001 (Neumann) and auto_004 (Black-Litterman + LW) both crashed inside `_optim_helpers` near the normalization step; HRP allocates by inverse *diagonal* variance only, sidestepping the numerical fragility that bit those iterations.
2. **Tilt-friendly** — after HRP gives a risk-coherent allocation, I multiply by `IS_Sharpe^0.5` to satisfy the user's "high-return / high-risk" mandate without abandoning the clustering structure. Then L1-rescale to a 0.60 gross-exposure target (squarely inside the [0.3, 0.9] band the spec demands).

Concentrate on top-25 after sign-aligning (IC), dedup at |ρ|>0.85, sort by IS Sharpe. Recursive bisection uses a 0.9/0.1 diagonally-shrunk sample cov for numerical safety.
