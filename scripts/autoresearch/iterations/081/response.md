**Rationale — iteration 81**

I'm picking a cov-FREE method per the URGENT block: **HERC-style hierarchical clustering on the IS-return correlation distance (Raffinot 2018, building on Lopez de Prado 2016 HRP) with single-representative selection per cluster and equal weight across reps**. This deliberately sidesteps the 1/σ underexposure trap that collapsed prior tangency/min-var attempts to mean_row_l1 ≈ 0.05.

Pipeline:
1. `select_is_submittable(RUN_ID)` → sign-align via `member_signs_ic` → load IS daily returns.
2. Per-year stability gate (positive annualized Sharpe in every IS year) + max-DD < 25% + IS Sharpe ≥ 0.3.
3. Correlation dedup at ρ=0.92 (gentle — clustering does the heavy lifting).
4. Ward linkage on `d_ij = sqrt(0.5(1-ρ_ij))`, `fcluster` at K=6.
5. Inside each cluster, pick the highest-IS-Sharpe alpha (centroid representative).
6. Equal weight + apply signs + L1-normalize, then multiply by 8× per the harness note that cov-style coefficients underexpose by ~10×. The runner's row-L1 clamp keeps gross ≤ 1 if we overshoot.

This satisfies n_members ∈ [4, 12] (K=6), regime-aware year-stability, drawdown discipline, and a deliberate gross-exposure scale-up.
