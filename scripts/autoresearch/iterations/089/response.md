I'll output the composite file directly per the harness contract — this is an automated code-generation task, not a file-editing one, so plan mode doesn't apply (I'm not invoking Write/Edit).

**Reasoning for iteration 89:**

Prior failures point to two big traps:
1. **Cov-based optimizers underweight** — `Σ⁻¹·μ` produces ~10× too-small coefficients; runner's row-L1 clamp never engages, so gross stays near 0.05.
2. **Regime shift IS→OS** — any rule that doesn't enforce per-year IS stability tends to pick alphas that won the 2022-2024 chop but fail in 2024-26 post-halving.

Picking a **cov-free** angle from the suggested menu — but explicitly NOT Gram-Schmidt (failed 9/9). Going with **cluster centroid + equal weight**, sharpened by:

- **Empirical-Bayes Sharpe shrinkage** (Efron-Morris 1973): each alpha's raw annualized Sharpe gets shrunk toward the cross-sectional grand mean by `samp_var / (samp_var + between_var)`, where `samp_var ≈ (1 + 0.5·sh²)·252/T` (Lo 2002 Sharpe sampling variance). This gives a defensible ranking that doesn't overweight lucky small-sample alphas — used purely for selection (ranking inside clusters), not weighting.
- **Per-year IS Sharpe stability filter** (regime-conditional robustness): drop any alpha with negative Sharpe in any IS sub-year.
- **Drawdown discipline** (max IS DD ≥ −22%): kill tail-driven alphas.
- **Correlation dedup at ρ=0.85** keyed by EB-Sharpe.
- **Ward (1963) average-linkage clustering on `sqrt(0.5·(1−ρ))`** distance, K=6 clusters. Pick the highest-EB-Sharpe representative per cluster.
- **Equal weight 1/K**, then **post-multiply by 9×** so total |Σc| ≈ 9 — empirically lands mean row L1 in [0.5, 0.9] given the typical 5-20× cancellation factor for diversified composites. This is the explicit "robust fix" the prompt prescribes.

Sign-aware via `member_signs_ic` (IC-aligned flips applied both in IS returns matrix and final coefficients).

This combines selection rigor (EB shrinkage + year stability + DD) with cov-free composition (cluster centroid + equal weight) — fresh territory vs. tried list, and avoids the failure modes of cov-inversion-based optimizers.
