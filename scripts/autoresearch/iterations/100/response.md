Looking at the spec, I need to focus on:
- **Cov-FREE methods** (the user has explicitly flagged cov-based optimizers as producing under-leveraged composites)
- **Concentrated n=5-8 members** (per the empirical leaderboard pattern)
- **Year-stability + drawdown discipline** (regime-shift mitigation)
- **Avoid Gram-Schmidt** (failed 9/9 prior attempts)
- **Fresh territory** — recent rejections used Neumann/tangency/NCO/BL/DRP/MaxDivRatio/Mean-CVaR/Kelly/HRP-bisection — but I haven't seen a **Ward-linkage + Sharpe-best-per-cluster centroid** approach with year-stability prefilter

Method: **HRP-style Ward clustering on IS-return correlation distance + Sharpe-best per-cluster centroid (cov-free), with year-stability + DD-discipline pre-filter, IC-sign alignment, correlation dedup, and gross-exposure rescale to mean row L1 ≈ 0.75.** This sits in the Lopez de Prado 2016 / Raffinot HERC tradition but skips the recursive-bisection risk inversion (which is where 1/σ-dilution sneaks back in), substituting a hard Sharpe-best centroid pick. Year-stability addresses the IS→OS regime shift; the gross rescale fixes the under-leverage trap.
