Picking a cov-free, regime-aware composition the spec explicitly recommends and that hasn't been tried in the listed iterations: Ward hierarchical clustering on correlation-distance → one centroid per cluster (highest IS Sharpe), then equal-weight. This bypasses the 1/σ-weighting trap of every cov-based optimizer (the documented blocker), keeps n in the sweet-spot range, and layers the year-stability + drawdown filters the leaderboard pattern endorses.

Citations in docstring: López de Prado (2016) HRP correlation-distance metric `√(0.5(1−ρ))`, and Raffinot (2018) HERC cluster-representative selection. The weighting itself is the cov-free **cluster-centroid + equal-weight + L1-rescale to target gross 0.70** recipe from the prompt.

Mechanism summary:
1. Pull SUBMITTABLE alphas, sign-flip via IC, load IS return matrix.
2. Drawdown gate < 22% (with relaxation fallback so we never starve the runner).
3. Per-year stability filter (positive mean in every IS year present).
4. Correlation dedup at ρ=0.88, keeping by IS Sharpe.
5. Ward linkage on √(0.5(1-ρ)) distance, fcluster into K=6 clusters.
6. Cluster representative = highest IS-Sharpe member in cluster.
7. Equal coefficients → `apply_signs` → `normalize_coefficients(·,"l1")` → multiply by TARGET_GROSS=0.70 (lifts mean row L1 into the [0.5,0.9] sweet spot without exceeding the runner's clamp).

Guards: helper calls wrapped where they have known fragility (iter 1 & 4 errored inside `select_is_submittable` / `normalize_coefficients`); fallbacks ensure ≥2 members are always returned and coef dict is always non-empty over the kept ids.
