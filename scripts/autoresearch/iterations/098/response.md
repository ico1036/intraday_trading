Reasoning — iteration 98:

The dominant failure pattern is cov-based optimizers (tangency/min-var/Neumann-tangency) producing 1/σ-weighted output that, after row-L1 normalization, leaves mean gross at ~5%. Top single alphas have OS Sharpe ~1.1; the best composite to date is 0.838 — strictly worse than holding the top alpha alone, because we're 95% in cash.

The leaderboard pattern says n=5-8, year-stable, DD-disciplined, dedup ~0.85 wins. The two near-winning "ingredient lists" are auto_013 (cluster median repr + eqweight) and auto_017 (year-stable + DD<25 + LW tangency). To explore an unexplored direction, I'll combine:

1. **Ward agglomerative HC** on the correlation distance `√(0.5(1−ρ))` (Ward 1963 + Lopez de Prado's NCO clustering insight) — produces compactly-shaped, balanced clusters.
2. **Max-IS-Sharpe centroid** per cluster (NOT median repr like auto_013, NOT min-var inside cluster like NCO) — gives the strongest available signal carrier within each correlation regime.
3. **Cov-FREE weighting**: pure equal weight 1/K (skip every form of Σ⁻¹). Gram-Schmidt is banned per the prompt; cluster-centroid+EW is the user-suggested untried path.
4. **Regime filters before clustering**: per-year IS Sharpe > 0 in every year (regime-robust) AND max IS DD < 25%.
5. **Native gross scaling to ~0.70 mean row L1** by estimating implied weight-stream gross via per-member σ.
6. Sign-aware via `member_signs_ic` so each contribution flows in its deployable direction.

This differs from every visible prior `auto_*` file: Ward agglomerative + cluster-centroid by max-Sharpe + cov-free equal weight + explicit native-exposure target = unexplored. K=6 is in the leaderboard sweet spot.
