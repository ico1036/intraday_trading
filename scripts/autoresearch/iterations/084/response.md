**Rationale.** Iter 001 (Neumann-tangency) crashed in `_optim_helpers`, and the leaderboard hint is unambiguous: cov-based optimizers underweight by ~10× and burn 95% of the risk budget on cash. I'm deliberately avoiding any matrix inversion this iteration. The user-suggested "cluster centroid + equal-weight" angle is the cleanest cov-free path and not in the tried list (auto_002 was NCO, auto_003 was HERC — both still cov-inversion families).

**Method.** Ward hierarchical clustering on correlation distance `d_ij = sqrt(0.5·(1 − ρ_ij))` (Mantegna 1999, Lopez de Prado 2016), cut at K=6, pick the highest-IS-Sharpe member as each cluster's centroid (a coarse "exemplar" selection), then 1/N over the K centroids. This preserves native member leverage (no Σ⁻¹ dilution), and the K=6 sits in the empirical sweet spot (n_members ∈ [5,8]).

**Regime-aware filters before clustering:**
1. `member_signs_ic` IC-aligned flipping (deploy-side sign).
2. Per-calendar-year IS Sharpe must be positive in **every** year (2022/23/24).
3. Max IS drawdown ≤ 25% (drawdown discipline).
4. `correlation_dedup` at ρ=0.88 keeping highest IS Sharpe (kills near-clones before clustering distorts).

**Gross-exposure fix.** L1-normalize to Σ|c|=1, then multiply by a fixed 5× scale. With K=6, that's ~0.83 per coefficient — enough to push the composite firmly into the runner's row-L1 cap region (1.0). Empirically prior cov-based attempts mean_row_L1 = 0.05; this should land 0.4–0.8.

**Why this should beat 0.41 OS Sharpe:** picks the *most diverse* high-quality regime-stable alphas (Ward groups correlated alphas together, centroid takes the best per group → orthogonality by construction), with native leverage preserved (no Σ⁻¹·1 collapse), no fragile inversion that can crash like iter 001.
