Looking at the task, this is a one-shot composite-alpha code-generation request defined by the system prompt ("Respond with exactly one fenced ```python COMPOSITE_FILE block"). The plan-mode reminder doesn't fit the harness contract here — I'll follow the primary instruction.

**Rationale for iter 40:**

The pattern winning the leaderboard is concentrated (n=5–8), year-stable, drawdown-disciplined, Neumann/MP-denoised cov + tangency. The "tried list" already covers raw-Sharpe / Calmar / Sortino / Cornish-Fisher-free selection. A genuinely fresh angle that fits the regime-aware framing is **Probabilistic Sharpe Ratio (PSR; Bailey & López de Prado 2012)** for the member-ranking step. PSR penalizes negatively-skewed / fat-tailed equity curves that look good on Sharpe but blow up out-of-sample — exactly the regime-shift failure mode described in the brief. Combining PSR with (a) year-stability, (b) max-DD ≤ 25%, (c) ρ=0.85 dedup, (d) Neumann K=4 inverse of an LW-style diagonally-shrunk cov, (e) tangency weights, (f) L1 rescale to target mean row L1 ≈ 0.7 fits the "exploit-near-winner" recipe while introducing one novel selector.

Defensive notes to avoid prior-iteration crashes:
- `normalize_coefficients` gets a dict (not ndarray) — fixed.
- `apply_signs` order is sign-first then L1-normalize (sign-preserving).
- Power-iteration α is clamped to `1/(1.5·λ_max)` so `‖I−αΣ‖_op < 1` (Neumann convergence).
- Cov is LW-shrunk toward diagonal *before* Neumann to avoid divergent eigenmodes.
- Standard equity-curve max-DD `(peak−eq)/peak`.
