Looking at the task: this is the automated composite-alpha researcher. The system prompt is explicit — respond with one fenced `python COMPOSITE_FILE` block, no tools, no plan workflow. I'll proceed with the code generation.

**Idea for iter 113 — cov-free Ward-cluster centroid with regime gating:**

- The "cov-free composition" hint family hasn't been explicitly tried (auto_013 used cluster *median*, not *max-IS-Sharpe* representative; that's a meaningful difference because median is robust-central while max-IS picks the genuinely best per-cluster).
- Combine with per-year IS Sharpe stability (regime-aware) + DD<20% discipline + dedup at ρ=0.85.
- Equal-weight K=6 centroids (n in the [4,12] sweet spot).
- Aggressive 10× post-L1-scale to break the gross-exposure ceiling that strangled past attempts to mean row L1 ≈ 0.05.
- Cites Lopez de Prado NCO (2019) cluster-representative family and Raffinot HERC (2018) hierarchical structure; the cov-free, max-IS-Sharpe representative is the novel twist over both.

Defensive fallbacks at every step so the file doesn't ERROR like iter 39/4/1.
