Looking at iteration 47, I'll pursue a **cov-FREE composition** angle that the brief explicitly flags as never-attempted: **Greedy Gram-Schmidt orthogonalization on residual Sharpe**. This bypasses the 1/σ-weighting trap that has compressed gross exposure to ~5% in prior iterations.

**Reasoning:**
- Prior cov-based tangency/min-var optimizers all produced `w ∝ Σ⁻¹·μ`, which after L1-normalization sits at mean row L1 ≈ 0.05 — leaving 95% of the risk budget unused. Best composite OS Sharpe = 0.838 < any top-10 individual alpha.
- Gram-Schmidt residual regression (classical Householder/QR, no covariance matrix needed) selects members by *information orthogonality* and weights them by their *residual* Sharpe — magnitudes natively ~O(1), not ~O(1/σ).
- Combined with regime-stability filter (positive Sharpe in every IS year — addresses the dominant IS→OS regime-shift failure mode), IS drawdown ≤ 25% (drawdown discipline), and correlation dedup at ρ=0.90.
- Stop adding members once residual Sharpe < 0.30 → automatic concentration in the n=4-10 sweet spot identified by leaderboard.
- Final L1 normalization × 0.85 puts coefficient aggregate at the high end of the brief's [0.30, 0.90] target band.
