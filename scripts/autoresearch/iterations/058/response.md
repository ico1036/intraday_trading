Looking at the spec, I need to pick a cov-FREE method (explicitly flagged as never-attempted territory). The user's leaderboard analysis shows tangency/MinVar style optimizers produce mean row L1 ≈ 0.05 (95% of risk budget wasted). Greedy Gram-Schmidt residual-Sharpe selection bypasses Σ⁻¹ entirely while explicitly maximizing orthogonal information — a classical matching-pursuit-style technique (Mallat & Zhang 1993; analogous to greedy forward selection in signal processing, and aligns with López de Prado's emphasis on selecting alphas by *marginal* contribution rather than raw IS Sharpe).

Strategy:
1. SUBMITTABLE pool → IC-aligned signs → sign-corrected IS returns matrix R.
2. Per-year IS stability filter (positive mean every IS sub-year) — regime robustness.
3. Drawdown filter (max IS DD < 25%).
4. Correlation dedup at ρ=0.85.
5. **Greedy Gram-Schmidt**: start with highest-IS-Sharpe; iteratively add candidate maximizing residual Sharpe after OLS-projecting onto already-selected members. Stop at n=8 or residual Sharpe < 0.30.
6. Weight by residual Sharpe (Gram-Schmidt amplitude proxy).
7. L1-normalize then scale by 10× to hit target mean row L1 ≈ 0.5–0.7 (counter the 1/σ ceiling trap).
8. Apply IC signs to deploy correctly via raw weight panels.

This is a fresh idea family (no NCO/HRP/HERC/BL/DRP/Neumann/Kelly/CSCV/anti-bias/Calmar etc.).
