Looking at the iteration history, the leaderboard winners cluster around **n=5-8 concentrated members + year-stability + low-DD + cov-regularized tangency + ρ-dedup**. Auto_001 tried Neumann-series, auto_017 used LW shrinkage with year-stability, auto_010 used James-Stein on Neumann.

A genuinely fresh regularization not yet tried is **explicit eigenvalue-clipping via a Tikhonov condition-number cap** (Tikhonov & Arsenin 1977): for sample cov Σ with eigendecomposition Σ = VΛVᵀ, clip λᵢ to [λ_max/κ, λ_max] before inversion. Unlike Neumann (truncates high-noise modes) and LW (uniform diagonal shrinkage), this directly bounds the conditioning of Σ⁻¹ and is sample-efficient when N is small.

I'll combine it with the proven leaderboard recipe (year-stable + DD<20% + top-8 + ρ=0.82 dedup) and mix the raw tangency with a Sharpe-proportional weighting (30/70) to avoid pure mean-variance fragility — Britten-Jones 1999 showed pure tangency on small samples is highly unstable; convex mixing with a robust prior shrinks toward it. Coefficient post-scale to mean row L1 ≈ 0.7 per the gross-exposure guidance.
