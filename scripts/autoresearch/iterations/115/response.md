...
I'll go with a **cov-FREE greedy Borda-rank composite** — fusing IS-Sharpe rank with an orthogonality rank (mean |corr| to already-picked) — and seed with sign-aligned IS returns. This avoids the 1/σ-weighting trap that strangled cov-based attempts, sits in the prompt's "never-attempted" list, and stacks the three ingredients that the leaderboard pattern says work: concentrated k=6, year-stability filter, drawdown discipline (max IS DD > −22%), and ρ=0.85 dedup. Final weights are equal-magnitude with sign flips from `member_signs_ic`, normalized to Σ|c|=1, then post-scaled by 5× to push mean row L1 into the 0.5–0.9 target band (since past attempts at Σ|c|≈1 mapped to gross ≈ 0.05–0.25, a 5× lift should land in the productive zone).

Method citation: **DeMiguel, Garlappi & Uppal (2009), "Optimal Versus Naive Diversification: How Inefficient is the 1/N Portfolio Strategy?", Review of Financial Studies 22(5)** — 1/N over a thoughtfully-screened set often beats Markowitz, and the Borda-fused screen (Borda 1781 rank aggregation, applied here à la rank-based active-set selection) supplies the screening. No Σ inversion, no shrinkage, no detoning — purely rank-and-orthogonality driven.
