...
1. Generate a composite alpha (attempt 85) that targets OS Sharpe ≥ 2.0
2. Avoid Gram-Schmidt (failed 9/9 priors), avoid covariance inversion (causes 1/σ-weighted tiny coefficients → mean row L1 ≈ 0.05)
3. Use a cov-free method with native exposure preservation
4. Apply the "consistent top-5 ingredients": n_members ∈ [5, 8], per-year IS Sharpe stability, DD < 20-25%, correlation-dedup at ρ ∈ [0.80, 0.90]

**Strategy choice:** Ward hierarchical clustering on `sqrt(0.5(1-corr))` distance (Lopez de Prado's quasi-diagonalization metric, also the canonical HRP/HERC distance, Raffinot 2018), cut at K=6 clusters, pick the per-cluster IS-Sharpe centroid representative, equal-weight. Pre-filter on per-year stability (positive Sharpe in every IS sub-year with ≥20 obs) + max-DD < 20%. Apply IC-based signs. Skip covariance inversion entirely (the documented blocker). Then aggressively rescale coefficients (×8 after l1-normalize) to push composite mean row L1 into the [0.5, 0.9] sweet spot rather than the 0.05 dead zone.

This is a fresh idea family — pure cov-free Ward-centroid composition has not been attempted; iter 003 used HERC (with cluster-internal risk-parity) and iter 013 used "stability_ir_cluster_median_repr" which uses a different distance + IR ranking. Ward + sqrt-corr distance + IS-Sharpe centroid + aggressive native-gross rescaling is novel.
