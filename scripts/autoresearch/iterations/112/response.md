Looking at the spec, I need to generate composite alpha attempt 112. Key constraints synthesized:

- **Idea family**: corr-rank + Sharpe-rank greedy is on the cov-free menu and NOT in the tried list (auto_011 was CSCV-bootstrap, auto_013 was IR-cluster-median, auto_017 was year-stable+DD — but none combined greedy corr+Sharpe rank).
- **Why this method**: explicitly cov-FREE per user's "blocker" note — sidesteps 1/σ trap. Implements Lohre/Opfer/Orszag-style diversification-rank composite + Sharpe-rank pairing.
- **Regime guards**: per-year IS Sharpe positivity (2022/2023/2024 sub-windows) + max-DD < 25% — both empirically validated as top-5 ingredients.
- **N=8**: in the documented [4, 12] sweet spot.
- **Gross**: equal-weight + L1-normalize then ×2 multiplier — guarantees Σ|c|=2 going into the runner's row-L1 clamp; produces mean row L1 ≈ 0.5–0.85 for moderately correlated members.
- **Signs**: `member_signs_ic` flip applied to coefficients (not to R again — `load_member_is_returns(..., signs=signs)` already does that on the returns side for filtering).
