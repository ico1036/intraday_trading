I'll generate a cov-free composite that combines three recommended angles: Ward hierarchical clustering on the HRP correlation-distance `d = √(0.5·(1−ρ))`, per-year IS-Sharpe regime-stability filter, and max-drawdown discipline — then pick the highest-IS-Sharpe alpha within each cluster (centroid representative) and equal-weight them. This explicitly bypasses every cov-inversion / tangency / min-var trap that produced sub-0.1 gross-exposure in prior attempts, and the post-scale by `GROSS_SCALE = 4.0` pushes Σ|c| ≈ 4 so the runner's L1 clamp produces a mean row L1 in the [0.6, 1.0] target zone instead of 0.05.

Method citations: HRP-style distance (Lopez de Prado 2016), Ward agglomerative linkage (Murtagh-Legendre 2014). Regime-stability filter is conceptually CSCV-informed (Bailey/Lopez de Prado 2014) — require positive sub-period Sharpe in every IS year as a cheap proxy. Cov-free 1/K weighting avoids the `w ∝ Σ⁻¹·1` 1/σ trap that capped exposure in iters 1-82.

Idea family ("Ward correlation-distance clustering + year-stability filter + cluster-centroid equal-weight, cov-free") not in the tried list. n=6 sits in the empirical sweet spot, DD floor at -25%, IC-sign alignment applied to deployable coefficients.
