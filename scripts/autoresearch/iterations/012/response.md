Looking at the iteration history, I see HRP, NCO, HERC, BL, DRP, Max-Div, mean-CVaR, fractional Kelly, James-Stein, and CSCV-bootstrap have all been tried — and all selected on top-of-pool IS Sharpe (or near-equivalents). Every one collapsed OS → ~0.

The prompt's most pointed (and counter-intuitive) hint is **middle-quintile selection**: rank 40-60% of SUBMITTABLE — anti-overfit logic from Bailey & López de Prado (2014, "Pseudo-Mathematics and Financial Charlatanism") and the PBO/CSCV literature. The top quintile is dominated by lucky overfits; middle ranks generalize cleaner because the selection operator itself adds less in-sample distortion. None of the prior 11 attempts touched this — it is fresh territory and squarely on the prompt's "selection bias is the dominant failure mode" thesis.

Design:
1. Pool = `select_is_submittable("run_2026_05_c")` (~471 alphas).
2. Apply IC signs before computing returns.
3. Compute per-member annualized IS Sharpe → keep the rank ∈ [0.35, 0.65] band (~150 alphas).
4. Correlation-dedup the middle band at |ρ|>0.85 (keep by Sharpe inside band).
5. Cap at 40 members via even spacing across the band's Sharpe rank (preserves the anti-bias spread, not just one end).
6. Equal-weight, `normalize_coefficients(..., "l1")` → Σ|c|=1, then × 0.65 to put mean row L1 in [0.4, 0.8].
7. `apply_signs(scaled, signs)` so IC-negative members contribute their deployable sign.

No matrix inversion (avoids the inversion-blowup that errored iters 001 & 004). No optimization at all — pure debiased baseline that isolates the *selection* hypothesis as the OS lever.
