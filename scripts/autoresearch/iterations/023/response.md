Looking at the tried list, I see year-stability (auto_017), HRP/HERC/NCO, BL, DRP, MaxDiv, CVaR, Kelly, James-Stein, CSCV, anti-bias, IC-stability cluster, ERC-bootstrap, PBO chunk, PC1-resid, mean-semivariance, EG-hedge, CCM, STARR, Calmar are all taken. The user emphasizes regime-shift between IS and OS as dominant failure, and concentrated n=6-10 picks dominate.

**Fresh angle (cited):** Use per-**quarter** Sharpe stability (finer than per-year auto_017) as a regime-robust filter, then build the small kept set via **Tikhonov-regularized tangency** (Tikhonov 1963) — distinct from LW shrinkage, NCO, HRP, MV, MinVol, MaxDiv variants. Tikhonov adds λI proportional to the trace/N, which both regularizes the ill-conditioned small-N covariance and softly tilts towards equal-risk when μ is noisy. Combined with sign alignment via IC, soft long-only clipping in the deployable-sign space, and a 0.70 gross-exposure target (per the row-L1 budget hint). Per-quarter granularity (≥70% positive-Sharpe quarters) gives ~8 distinct regime checkpoints across 2022-2024 IS rather than 3 years — stricter regime-conditional robustness, but with a relaxed-fallback so the filter never returns empty.
