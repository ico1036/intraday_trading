...
**Strategy choice:** The spec explicitly flags **Greedy Gram-Schmidt residual-Sharpe** as a "never attempted, cov-free" recommended method. The orthogonal-information formulation (analogous to QR-based factor selection in Cochrane 2005, ch. 13) bypasses the `Σ⁻¹` trap that under-leverages prior attempts. Selection rule pairs this with the empirical winning pattern: per-year IS stability + drawdown discipline + n ≈ 7 + aggressive gross post-scale.

**Concretely:**
1. Submittable pool → sign-align by IC → drop alphas with negative Sharpe in any IS year or max DD ≥ 25%
2. Anchor = highest IS-Sharpe survivor. Then iteratively project out picked alphas via QR-style orthogonalization on the IS returns matrix; next pick = candidate with highest residual Sharpe.
3. Stop at n=7 or residual Sharpe < 0.30 (no new orthogonal info).
4. Weight each pick by its residual Sharpe, sign-flip, L1-normalize to Σ|c|=1, then **multiply by 10×** (per the explicit "5-15× post-scale" instruction) so the row-L1 clamp in `_runner.py` actually engages and gross exposure lives in [0.5, 0.9] instead of the 0.05 dead zone.
