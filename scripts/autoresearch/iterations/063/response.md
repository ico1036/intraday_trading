Looking at this task, I need to generate iteration 63 of the composite alpha researcher. There are conflicting system reminders (Plan mode vs. the explicit code-gen contract in the user message), but the harness contract is unambiguous: "Respond with exactly one fenced ```python COMPOSITE_FILE block". This is a non-interactive automated pipeline — creating a plan file would break it. I'll produce the code block.

**Reasoning / method choice:**

Prior attempts overwhelmingly used cov-based optimizers (tangency, min-var, Neumann), and the explicit failure analysis says these produce `w ∝ Σ⁻¹·μ`, which after row-L1 clamp leaves 95% of the risk budget unused (mean row-L1 ≈ 0.05). The user's explicit guidance is to try **cov-free composition** this iteration.

I'll use the **rank-sum greedy composition** (combined Sharpe-rank + orthogonality-rank score; see the menu) — a method *not yet tried* in any of auto_001..062 — with these regime-aware ingredients drawn from the leaderboard pattern:

- **Year-stability filter** (Lopez de Prado-style regime robustness): keep only alphas with positive mean return in each IS sub-year (2022/2023/2024) — separates genuinely robust members from cycle-lucky ones across the 2022 bear → 2023 chop → 2024 ETF-rally regimes.
- **Drawdown discipline** (max IS DD < 25%) — filters tail-luck alphas.
- **Greedy rank-sum** (Sharpe-rank + ‖corr‖-rank): mirrors information-theoretic feature selection (mutual-information-greedy a la Battiti '94, repurposed for portfolio member picking). Bypasses Σ⁻¹ entirely.
- **Sign alignment** via `member_signs_ic` so the deployable direction is correct before greedy selection.
- **Equal-weight 1/N** over n=6 selected members (concentrated; auto_002 winner had n=6).
- **Explicit gross-exposure rescale** to target mean row-L1 ≈ 0.7 — the critical fix the user flagged.

This is genuinely fresh territory (no cov inversion, no Σ⁻¹ flavor, no max-div ratio, no HRP/HERC, no Black-Litterman, no Michaud) and addresses the root cause (gross-exposure ceiling) head-on.
