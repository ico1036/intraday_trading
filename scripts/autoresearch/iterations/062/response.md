I'll generate iteration 62. The recent leaderboard pattern points to concentrated (n=5-8), year-stable, drawdown-disciplined picks with cov-free composition. The user explicitly flagged that cov-based optimizers underweight by ~10× (1/σ trap), and Gram-Schmidt has failed 9/9. So I'll pick the **correlation-rank + Sharpe-rank greedy blend** (cov-free method #3), paired with the empirically-validated leaderboard ingredients:

- Per-year IS Sharpe > 0 across every sub-year (regime conditioning — Lopez de Prado 2018 *Advances in Financial ML*, chapter 7).
- Max IS drawdown < 20% discipline.
- Correlation dedup at ρ=0.85 (Lopez de Prado 2016 HRP distance) using IS Sharpe as keep-metric.
- Greedy 50/50 blend of `rank(IS-Sharpe)` and `rank(mean |ρ| with prior picks)` — this is a quasi-orthogonal extension of Choueifaty (2008)'s maximum-diversification idea without inverting Σ.
- Equal-weight 1/6 on the kept set, signs from `member_signs_ic` (Grinold-Kahn 1995 IC-orientation), then a vol-aware post-scale to target mean row L1 ≈ 0.65 — avoiding the gross-exposure trap.

This is a fresh angle: pure rank-based selection with no cov inversion, no shrinkage, no Neumann series. It explicitly attacks the "1/σ weighting trap" the harness flagged as the dominant blocker.
