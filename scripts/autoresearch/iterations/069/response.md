Looking at the brief, the dominant failure pattern is regime shift IS→OS and the gross-exposure ceiling. Top performers cluster at n=5-8 with year-stability + DD discipline. The user also explicitly opened the door to cov-FREE composition (cluster centroid + equal-weight) to bypass the 1/σ trap.

For iter 69, I'll combine three regime-robustness filters (per-year IS Sharpe ≥ 0, max IS DD ≤ 25%, IS Sharpe ≥ 0.4) with **Ward hierarchical clustering** on the IS-return correlation distance (Murtagh 1985 / Raffinot 2018-adjacent cluster-centroid heuristic), then pick the single best-IS-Sharpe representative per cluster and equal-weight K=6. This is a deliberately cov-FREE method — no inversion, no shrinkage — which addresses the gross-exposure complaint by NOT using Σ⁻¹·μ-style weights. Sign-alignment via IC, then apply the user's recommended `sigma_a × |c|` proxy to rescale to mean row L1 ≈ 0.70.

Method family is new in this run (no Ward / no cluster-centroid attempt logged in tried-list). Avoiding Gram-Schmidt (failed 9/9) and middle-quintile anti-bias (OS-negative).
