Looking at the iteration 106 context: prior cov-based optimizers underweight by 10× (mean row L1 ~0.05 vs target 0.5-0.9). Gram-Schmidt failed 9/9. Top leaderboard pattern is n=5-8, per-year stability, DD<25%, dedup at ρ≈0.85. The cov-FREE "Cluster centroid + equal-weight" path explicitly listed in the prompt has not been attempted yet.

**Mechanism:** Ward hierarchical clustering on `dist = √(0.5·(1−ρ))` (Lopez de Prado 2016 correlation-distance metric) partitions sign-aligned IS returns into K=6 buckets. The single highest-IS-Sharpe alpha per cluster is taken as the centroid — this avoids covariance inversion entirely (sidestepping the 1/σ underweighting trap) while still capturing the diversification structure. Pre-filters: per-calendar-year Sharpe positivity (regime stability across 2022/2023/2024) and max IS drawdown < 25% (tail discipline). After L1 normalization the coefficients are scaled by 10× — empirically the runner's row-L1 clamp will produce mean row L1 in the target [0.5, 0.9] band rather than the anemic 0.05.

This combines four leaderboard-winning ingredients (concentrated n=6, year-stable, DD<25%, dedup at 0.85) with the genuinely-unexplored cov-FREE Ward-centroid skeleton.
