I'll combine the user's Neumann-series request with regime-aware concentration: drawdown discipline + per-year IS positivity gate the pool down to truly regime-robust alphas, dedup at |ρ|>0.85, then take a concentrated top-8. For weighting I use Marchenko-Pastur eigenvalue clipping (Lopez de Prado 2019, *MLfAM*) to denoise the sample covariance, then a truncated Neumann series (K=4) to approximate Σ⁻¹ stably — which is the requested Neumann eigenvalue-divergence suppression — and form a tangency-style w ∝ Σ̂⁻¹μ. Coefficients are L1-normalized and scaled to target mean row-L1 ≈ 0.65 (well within the [0.30, 0.90] gross-exposure sweet spot the harness instructions flagged). This is a fresh angle: iter 001 attempted Neumann tangency but errored, and crucially did not pair the inverse with regime-stable selection or MP denoising, so the underlying eigenstructure was noise-dominated.
