Approach: **Greedy Gram-Schmidt residual-Sharpe orthogonalization** — a cov-FREE method (per the user's "never attempted" menu) that addresses the gross-exposure ceiling problem by skipping inverse-cov 1/σ weighting entirely. Pre-filter on per-year IS stability and DD<25% (matches the winning leaderboard pattern), dedup at ρ=0.85, then iteratively grow the basket by picking the candidate whose IS-residual (after regressing on already-selected) has the highest Sharpe. Weight each member by its incremental residual-Sharpe, normalize L1, then rescale to target mean row L1 ≈ 0.65 to avoid the 5%-gross underexposure trap.

Mechanism / lit:
- Classical Gram-Schmidt residualization for orthogonalization
- Residual information-ratio gating (information coefficient theory à la Grinold-Kahn) — each addition must clear an annualized Sharpe of 0.3 on the residual
- Year-stability + DD-discipline filter as a regime-conditional robustness proxy (per CRITICAL section: IS→OS regime shift is the dominant failure)
- Cov-free composition preserves native member leverage (no `Σ⁻¹·μ` shrinking of gross)

Idea family check: not in tried list (auto_016 was PC1 single-projection; auto_005 was eigenbasis DRP; none used iterative Gram-Schmidt residual-Sharpe gating with year-stability pre-filter).
