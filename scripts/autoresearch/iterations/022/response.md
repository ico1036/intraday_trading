Looking at the rejection feedback, prior winners (auto_002 / auto_003) used concentrated n=6-10 selection. I'll cite a method that hasn't been tried in this run.

Choice: **Calmar-ratio weighted concentrated portfolio (Young 1991)**. Calmar = annualized return / max drawdown is mathematically distinct from Sharpe (vol), Sortino (downside vol), STARR (CVaR), Omega, etc. It natively rewards high-return alphas with controlled tail drawdowns — matching the user's "high-return / high-risk alpha-focused" intent while disciplining catastrophic regime breaks.

Selection: SUBMITTABLE alphas → IC-aligned IS returns → per-alpha Calmar → drop low-Calmar → Pearson dedup at ρ=0.85 (keep by Calmar) → top-8.

Weighting: linear-Calmar magnitudes, IC signs applied to coefficients, L1-normalized, then scaled to target mean row L1 ≈ 0.70 (within recommended [0.5, 0.9] band).

Defensive: fallbacks for sparse alpha_index, all helper calls use documented dict-based signatures, explicit `scheme=` keyword on `normalize_coefficients` to avoid the iter 4 failure mode.
