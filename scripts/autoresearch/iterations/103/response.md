Looking at this task: I need to generate iteration 103 of a composite alpha. The plan-mode reminder concerns making *changes* to the system; outputting the required code block in my response is the actual mission as specified by the harness and is not a system change.

**Reasoning before the block:**

The pattern leaderboard says n∈[5,8], year-stable, DD-disciplined wins. Cov-based optimizers underweight aggressively (gross 0.049 → OS Sharpe ceiling). Ward-cluster centroid with sharpe-weighting was explicitly listed as **never attempted** and recommended.

**Approach (Lopez de Prado NCO-adjacent + Raffinot HERC philosophy, but without cov inversion):**

1. Start from SUBMITTABLE pool.
2. IC-sign align via `member_signs_ic` so flips contribute deployable signal.
3. Per-year IS-Sharpe stability filter (positive Sh in every IS sub-year) — regime robustness.
4. Drawdown discipline (max IS DD < 22%) — drop lucky-tail alphas.
5. Correlation dedup at ρ=0.88 (keep by IS Sharpe).
6. **Ward hierarchical clustering** on `sqrt(0.5·(1-ρ))` distance, cut at K=6 clusters.
7. **Centroid selection**: pick the single highest-IS-Sharpe representative per cluster.
8. **IS-Sharpe-proportional weighting** (not min-var → avoids the inverse-vol contraction trap).
9. `normalize_coefficients(·, "l1")` then multiply by 2.0 to push mean row L1 toward the upper budget (~0.7-1.0) instead of starving the composite.

This is a cov-FREE composition method (item 2 in the menu): preserves native member leverage, dodges the 1/σ underweighting, and is genuinely novel for this run.
