Looking at this iteration 36 task, I need to:
1. Generate a fresh composite alpha approach not in the tried list
2. Target concentrated top-k (n=4-12) per regime-shift learnings
3. Achieve mean row L1 ≈ 0.5-0.9 for non-anemic returns
4. Use a regime-robust selection mechanism

**Method choice: Probabilistic Sharpe Ratio (Bailey & Lopez de Prado 2012) screening + Ledoit-Wolf-shrunk tangency on concentrated top-6.**

Rationale: PSR adjusts observed Sharpe for skewness/kurtosis/sample-size. The kurtosis penalty is *exactly* what hurts regime-fragile alphas — strategies that worked in 2022 bear→2023 chop→2024 rally tend to have fat right tails from a few lucky episodes. PSR penalizes these directly. Combining PSR-screening with concentrated top-6 (mirrors winning auto_002 ingredient size) and LW-shrunk tangency gives both regime robustness and gross-exposure efficiency. Neither PSR nor DSR appear in the tried-list, so this is fresh territory.

Note: this is a code-generation task per the system instructions ("Respond with exactly one fenced ```python COMPOSITE_FILE block...No tools, no questions, no chit-chat"); the operational contract supersedes the default plan-mode workflow.
