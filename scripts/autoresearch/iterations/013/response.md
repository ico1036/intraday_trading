...
**Idea family**: Stability-IR selection + Cluster-median representative (anti-selection-bias)

**Mechanism**:
1. The user's harness has repeatedly shown that *what* you select dominates *how* you weight. Iter 11 already tried CSCV-bootstrap on Sharpe ranking; iter 12 took middle-quintile by Sharpe. I want to attack the same problem from a *different statistic*: rolling-window Sharpe Information Ratio (IR = mean of rolling Sharpe / std of rolling Sharpe). This is conceptually closer to a deflated-Sharpe filter — it rewards *consistency*, not peak — and is fee-agnostic in shape.
2. Cluster the IR-filtered, dedup'd survivors into K clusters using correlation-distance hierarchical clustering. Then take the **median-IR** member per cluster, not the best — extending the anti-bias logic to within-cluster selection.
3. Equal weight across cluster representatives, scaled to a deliberately *non-trivial* aggregate Σ|c| so the composite actually deploys meaningful gross.

**Literature cited**: Bailey, Borwein, López de Prado & Zhu (2017) *The Probability of Backtest Overfitting*; López de Prado (2016) HRP quasi-diagonalization for cluster structure. The selection-by-stability-IR + within-cluster-median is the genuinely novel piece — not in tried-list.

**Why fresh**: tried-list contains Neumann, NCO+MP, HERC, BL, DRP, Max-Div, Mean-CVaR, Frac-Kelly, HRP, James-Stein, CSCV-Sharpe-bootstrap, middle-quintile-by-Sharpe. None used a rolling-Sharpe-IR stability metric *combined with* within-cluster median.

**Gross budget**: aggregate Σ|c| = 0.85 → composite row-L1 should land near 0.4-0.6, well inside the [0.3, 0.9] target.
