Looking at the spec and recent attempt history, I need to:

1. Avoid families already tried: Neumann/NCO/HERC/BL/DRP/MaxDiv/CVaR/Kelly/HRP/James-Stein/CSCV/antibias/PBO/PC1-resid/year-stable-tangency (auto_017)/mean-semivariance/macro-hedge/CCM/STARR/Calmar
2. Address the dominant failure mode: **regime shift IS→OS** — point-estimate tangency overfits the IS sample
3. Target ~8 members at mean row L1 ≈ 0.65

**Mechanism choice: Michaud Resampled Efficient Frontier (1998/2008)** — bootstrap the IS returns matrix B times, compute Ledoit-Wolf-shrunk tangency on each resample, then *average* the weight vectors. This is a frequentist Bayesian-flavored regularizer that produces weights stable to perturbations of the IS sample. Unlike auto_017's point-estimate tangency, the averaged weights collapse toward members that are *consistently* optimal across many sub-samples — which is the closest analog to "regime-robust" we can get without OS data.

I add a **stationary block bootstrap** (Politis-Romano 1994) with block length 5 to preserve short-term autocorrelation in daily returns. Selection uses a different combo than auto_017: just top-IS-Sharpe + dedup at ρ=0.85, letting the bootstrap averaging do the regime-robustness work rather than relying on per-year hard gates that already failed in auto_017. I also blend each bootstrap's `Σ⁻¹μ_boot` with `Σ⁻¹μ_full` (Stein-shrinkage flavor on the mean) — pure bootstrap μ is too noisy with T~600.
