Reasoning: The user's explicit ask is Neumann-series eigenvalue-divergence suppression for MinVol, combined with high-return/high-risk concentration. Prior iterations show extreme concentration (n=5-10) beats large diversified baskets, and year-stability + drawdown filters defend against the IS→OS regime shift. I'll implement pure **Minimum-Variance via truncated Neumann-series Σ⁻¹** (w ∝ Σ⁻¹·𝟙, no μ — avoids mean-estimate error) on a **year-stable, drawdown-disciplined top-5 IS-Sharpe concentrated set**, dedup at |ρ|≤0.85, sign-aligned via IC. Neumann series is appropriate here: with α = 1.5/λ_max(Σ̃) and K=5 we suppress the highest-noise eigenmodes without ever inverting a near-singular Σ; Ledoit-Wolf-style diagonal shrinkage stabilizes the spectrum before truncation. The combination of (regime-robust selection) × (MinVar weighting) × (gross-exposure target 0.7) targets the failure mode where tangency/μ-driven combos transfer poorly out-of-sample. Differentiator vs auto_017 (lw_tangency, k=8) and auto_001 (tangency, k=60): pure MinVar, k=5, and explicit Neumann-α calibrated by power iteration.
