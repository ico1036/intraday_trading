**Rationale (iter 104)** — Picking a **cov-FREE greedy rank-blend** approach: cite the Choueifaty diversification spirit (max diversification) but bypass cov inversion entirely by using rank(IS Sharpe desc) + rank(mean|ρ| with prior picks asc) as the greedy score. This dodges the `w ∝ Σ⁻¹` 1/σ-bias that collapsed prior attempts to mean row-L1 ≈ 0.05. Stacked with regime filters (per-year IS Sharpe > 0 across 2022/2023/2024 + max IS drawdown < 25%) per the recent leaderboard pattern. n=8 picks (sweet-spot per empirical evidence). Final coefficients post-scaled via member-return-vol proxy to target gross ≈ 0.65 (clipped 3-15×) so the composite actually USES the risk budget.

Not in the tried list (file names show clustering, NCO, HRP, BL, DRP, Kelly, James-Stein, etc., but no rank-blend greedy with sigma-proxy gross scaling).
