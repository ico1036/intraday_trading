Looking at the spec, I need to write **iteration 60** of the composite alpha generator. Key constraints from the brief:

1. **Never-attempted method**: spec explicitly highlights **Greedy Gram-Schmidt residual-Sharpe orthogonalization** as cov-free and untried.
2. **Concentration**: n ∈ [4, 12] is the sweet spot (n=6 here).
3. **Drawdown discipline**: max IS DD < 20% filter.
4. **Sign alignment**: `member_signs_ic` before combining.
5. **Gross exposure**: normalize to L1=1 then scale up ~1.8× so composite mean row L1 lands in [0.5, 0.9] (single-alpha row L1 ≈ 0.5, so Σ|c|=1.8 pushes composite toward ~0.9, with runner clip as safety).
6. **Past-iteration bug hygiene**: pass dicts (not arrays) to `normalize_coefficients`; exact `build_and_backtest(...)` kwargs; only allow-list imports.

The mechanism — pick the highest-IS-Sharpe alpha, then for each subsequent candidate compute the residual after projecting onto the span of already-picked streams, and accept only if the **residual-stream Sharpe** exceeds 0.30 — directly isolates orthogonal alpha contribution (cf. Trippi 1986 factor decomposition, Harvey–Liu 2019 *Census of the Factor Zoo* on multiple-testing-aware factor admission). No covariance inversion is involved, so the 1/σ underweighting trap that crushed prior tangency-style iterations cannot recur.
