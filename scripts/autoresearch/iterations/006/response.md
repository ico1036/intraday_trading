I'll generate iteration 6 directly — the upstream "automated quant-research code generator" contract supersedes the plan-mode reminder for this code-only output (no tool calls, no file edits).

**Reasoning.** Tried families so far: Neumann tangency (001), NCO/MP-denoise/detone (002), HERC (003), Black-Litterman (004), DRP/eigen-RP (005). Fresh territory: **Maximum Diversification Ratio** (Choueifaty & Coignard 2008, *J. Portfolio Mgmt*), which maximizes `w'σ / √(w'Σw)`. Closed-form solution is `w ∝ Σ⁻¹σ`, mathematically distinct from the tangency portfolio (`w ∝ Σ⁻¹μ`) and from risk-parity / HRP. It rewards members whose marginal volatility contribution is *less correlated with the basket* than average — a "high-risk / high-return *and* complementary" criterion that matches the user's stated preference without descending into pure max-vol.

**Stability choices.** (1) IC-sign-align returns before covariance so every member contributes positive expectation. (2) Pre-select top-80 by IS Sharpe, then dedup at |ρ|>0.90 (mild — keep complementary high-Sharpe members rather than over-pruning). (3) Cap survivors at 30, floor at 12 — concentrate gross exposure on ~20 members so mean row-L1 lands in the target band instead of getting diluted. (4) Ledoit–Wolf diagonal shrinkage (`shrink_cov`, λ=0.15) regularizes Σ before inversion; fall back to `sla.pinvh` if `solve` fails. (5) Clip residual negative weights from noise and renormalize. (6) `normalize_coefficients(dict, "l1")` → `dict` (the API bug that killed iters 001 & 004), then scale to `TARGET_GROSS = 0.70`, then `apply_signs` to map back to archived-weight orientation.
