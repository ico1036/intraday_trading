Looking at the situation: this is a pure code-generation task per the system prompt — the harness extracts a fenced block from my text output. No file edits or system changes happen on my end, so I'll produce the required output.

**Rationale:** Recent iterations (001, 004, 039) errored on helper-API misuse (passing ndarrays instead of dicts to `normalize_coefficients`, etc.). The leaderboard analysis is unambiguous: cov-based optimizers chronically underweight (mean row-L1 ~0.05 → ~0.25), leaving 75-95% of the risk budget unused, so the composite under-performs single best alphas. The user explicitly flagged cov-free composition as fresh territory and called out Gram-Schmidt as already-failed (9/9).

**Method choice:** *Ward hierarchical clustering on correlation distance + per-cluster top-IS-Sharpe representative + sign-aligned equal-weight with native-gross amplification.* Cites Raffinot 2018 HERC for the clustering primitive and Feng-Giglio-Xiu 2020 "taming the factor zoo" for the cluster-representative selection step. This is genuinely cov-free (no Σ⁻¹ anywhere), matches the winning recipe (k≈6 members, year-stability filter, DD discipline, ρ≈0.88 dedup), and explicitly amplifies the L1 coefficient sum well past 1 so the runner's row-L1 clamp engages — pushing typical gross into the target [0.5, 0.9] band instead of dribbling at 0.05.

**Stability gates before clustering:** positive Sharpe in every IS calendar year + max IS DD < 25% + ρ=0.88 dedup. Then Ward(K=6), pick the highest-IS-Sharpe alpha in each cluster, equal-weight, IC-flip signs via `member_signs_ic`, then scale total |c| to ≈8 so runner clamping (not a tiny optimizer) sets the gross.
