Looking at iteration 61's brief, I'm picking a **cov-FREE composition method** the prompt flags as never-attempted: **Greedy Gram-Schmidt residualization on IS returns**. The argument:

1. Every prior tangency/min-var attempt produced `Σ|w| ≪ 1` because `w ∝ Σ⁻¹·μ` is 1/σ-weighted by construction — leaves 95% of the gross-exposure budget on the table.
2. Gram-Schmidt scores each new candidate by the **Sharpe of its residual** after projecting out already-chosen members. This explicitly rewards *orthogonal alpha signal* (information not already in the basis), not low variance.
3. Equal-weighting the kept set then rescaling Σ|c|→8 preserves native member leverage; the runner's row-L1 clamp handles tail spikes.
4. Layer in the empirically-winning ingredients: per-year IS-Sharpe stability (regime check across 2022/2023/2024), max IS DD < 25% (luck filter), correlation dedup at ρ=0.85, IC-sign alignment.

This is the user-suggested "Greedy Gram-Schmidt orthogonalization" from the cov-FREE menu, combined with the empirically-winning n∈[4,8]/year-stable/DD-disciplined ingredient stack.
