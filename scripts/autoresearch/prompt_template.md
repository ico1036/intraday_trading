# Composite Alpha Researcher — Iteration {N}
...
Build a composite alpha that achieves **OS daily Sharpe ≥ {target_os_sharpe}**
on archived run `{run_id}` (real fees + slippage via the canonical
backtester). You are attempt **{N}** of at most {max_iterations}.
...
```python
COMPOSITE_ID = "{composite_id}"      # exact string, do not change
...
`alpha_index` columns (os_* already stripped): {alpha_index_columns}

## Alpha pool summary (run `{run_id}`)

- Total alphas with IS metrics: **{n_alphas}**
- SUBMITTABLE-labeled (IS-only gate): **{n_submittable}**
- Median IS Sharpe across pool: **{is_sharpe_median:.3f}**
- 90th-percentile IS Sharpe: **{is_sharpe_p90:.3f}**
- Top-10 IS Sharpe (sample):
{top_alphas_table}

**Note:** For run `{run_id}`, alpha_index.csv may be sparse — prefer
filesystem-scanning helpers with the explicit run id:

```python
RUN_ID = "{run_id}"  # hard-code inside select_members / member_weights
...
## Recent rejection feedback (last {n_recent_rejects} attempts)
...
better mathematical justification for hitting OS Sharpe ≥ {target_os_sharpe}.
...
- [ ] `COMPOSITE_ID = "{composite_id}"` exactly.
