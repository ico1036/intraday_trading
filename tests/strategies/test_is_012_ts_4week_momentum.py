...
from intraday.strategies.multi.ts4week_momentum_strategy import Ts4weekMomentumStrategy
...
def test_warmup_returns_none():
    s = Ts4weekMomentumStrategy(symbols=["BTCUSDT", "ETHUSDT"], lookback_bars=2, history_window=3, rebalance_bars=1)
    assert feed(s, [{"BTCUSDT": {"close": 100.0}, "ETHUSDT": {"close": 100.0}}]) is None


def test_long_high_z():
    s = Ts4weekMomentumStrategy(symbols=["BTCUSDT", "ETHUSDT"], lookback_bars=2, history_window=3, rebalance_bars=1, entry_z=1.0, max_weight=0.4)
    rows = [
...
def test_finite_weights():
    s = Ts4weekMomentumStrategy(symbols=["BTCUSDT", "ETHUSDT", "SOLUSDT"], lookback_bars=2, history_window=3, rebalance_bars=1, entry_z=0.5, max_weight=0.3)
    rows = [
...
def test_single_symbol():
    s = Ts4weekMomentumStrategy(symbols=["BTCUSDT"], lookback_bars=2, history_window=3, rebalance_bars=1)
