...
from intraday.strategies.multi.xs_daily_reversal_strategy import (
    XsDailyReversalStrategy,
)
...
def test_warmup_returns_none():
    strategy = XsDailyReversalStrategy(
...
def test_long_loser_short_winner_after_warmup():
    """Direction inverted vs xs_daily_momentum."""
    strategy = XsDailyReversalStrategy(
        symbols=["BTCUSDT", "ETHUSDT"],
        lookback_bars=2,
        rebalance_bars=1,
    ...
    # BTC is winner → SHORT; ETH is loser → LONG (inverted from momentum)
