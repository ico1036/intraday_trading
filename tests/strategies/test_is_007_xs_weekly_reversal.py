...
from intraday.strategies.multi.xs_weekly_reversal_strategy import (
    XsWeeklyReversalStrategy,
)
...
def test_warmup_returns_none():
    strategy = XsWeeklyReversalStrategy(
...
def test_long_loser_short_winner():
    strategy = XsWeeklyReversalStrategy(
        symbols=["BTCUSDT", "ETHUSDT"],
        lookback_bars=2,
        rebalance_bars=1,
    ...
    assert btc.side == Side.SELL  # winner shorted (reversal)
